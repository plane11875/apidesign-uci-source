...
typedef struct b64_struct {
    ...
    UCI_ENCODE_CTX *base64;
    unsigned char buf[UCI_ENCODE_LENGTH(B64_BLOCK_SIZE) + 10];
...
static int b64_free(BIO *a)
{
    ...
    UCI_ENCODE_CTX_free(ctx->base64);
    OPENSSL_free(ctx);
    BIO_set_data(a, NULL);
    BIO_set_init(a, 0);
    ...
    while (outl > 0) {
        ...
        if (i <= 0) {
            ...
            if (!BIO_should_retry(next)) {
                ...
                if (ctx->tmp_len == 0) {
                    if (UCI_DecodeFinal(ctx->base64, NULL, &num) < 0)
                        ret_code = -1;
                    UCI_DecodeInit(ctx->base64);
                }
                ctx->cont = ret_code;
        ...
        if (ctx->start && (BIO_get_flags(b) & BIO_FLAGS_BASE64_NO_NL) != 0) {
            ...
            for (j = 0; j < i; j++) {
                ...
                k = UCI_DecodeUpdate(ctx->base64, ctx->buf, &num, p, (int)(q - p));
                UCI_DecodeInit(ctx->base64);
                if (k <= 0 && num == 0) {
                    p = q;
                    continue;
        ...
        i = UCI_DecodeUpdate(ctx->base64, ctx->buf, &ctx->buf_len,
                             ctx->tmp, i);
        ctx->tmp_len = 0;
        /*
        ...
        if ((BIO_get_flags(b) & BIO_FLAGS_BASE64_NO_NL) != 0) {
            if (ctx->tmp_len > 0) {
                ...
                if (ctx->tmp_len < 3)
                    break;
                ctx->buf_len =
                    UCI_EncodeBlock(ctx->buf, ctx->tmp, ctx->tmp_len);
                if (!ossl_assert(ctx->buf_len <= (int)sizeof(ctx->buf))) {
                    ERR_raise(ERR_LIB_BIO, ERR_R_INTERNAL_ERROR);
                    return ret == 0 ? -1 : ret;
                }
                ...
                n -= n % 3;
                ctx->buf_len =
                    UCI_EncodeBlock(ctx->buf, (unsigned char *)in, n);
                if (!ossl_assert(ctx->buf_len <= (int)sizeof(ctx->buf))) {
                    ERR_raise(ERR_LIB_BIO, ERR_R_INTERNAL_ERROR);
                    return ret == 0 ? -1 : ret;
                }
            ...
            if (!UCI_EncodeUpdate(ctx->base64, ctx->buf, &ctx->buf_len,
                                  (unsigned char *)in, n))
                return ret == 0 ? -1 : ret;
            if (!ossl_assert(ctx->buf_len <= (int)sizeof(ctx->buf))) {
                ERR_raise(ERR_LIB_BIO, ERR_R_INTERNAL_ERROR);
...
static long b64_ctrl(BIO *b, int cmd, long num, void *ptr)
{
    ...
    case BIO_CTRL_WPENDING:    /* More to write in buffer */
        ...
        if (ret == 0 && ctx->encode != B64_NONE
            && UCI_ENCODE_CTX_num(ctx->base64) != 0)
            ret = 1;
        else if (ret <= 0)
            ret = BIO_ctrl(next, cmd, num, ptr);
        break;
 ...
 again:
        ...
        if (BIO_get_flags(b) & BIO_FLAGS_BASE64_NO_NL) {
            if (ctx->tmp_len != 0) {
                ctx->buf_len = UCI_EncodeBlock(ctx->buf,
                                               ctx->tmp, ctx->tmp_len);
                ctx->buf_off = 0;
                ctx->tmp_len = 0;
                goto again;
                   ...
                   && UCI_ENCODE_CTX_num(ctx->base64) != 0) {
            ctx->buf_off = 0;
            UCI_EncodeFinal(ctx->base64, ctx->buf, &(ctx->buf_len));
            /* push out the bytes */
            goto again;
        }
