...
static void uci_signature_free(void *data)
{
    UCI_SIGNATURE_free(data);
}

static int uci_signature_up_ref(void *data)
{
    return UCI_SIGNATURE_up_ref(data);
}

static UCI_SIGNATURE *uci_signature_new(OSSL_PROVIDER *prov)
{
    UCI_SIGNATURE *signature = OPENSSL_zalloc(sizeof(UCI_SIGNATURE));
...
static void *uci_signature_from_algorithm(int name_id,
                                          const OSSL_ALGORITHM *algodef,
                                          OSSL_PROVIDER *prov)
{
    const OSSL_DISPATCH *fns = algodef->implementation;
    UCI_SIGNATURE *signature = NULL;
    const char *desc;
    ...
    if ((signature = uci_signature_new(prov)) == NULL) {
    ...
    if (ctxfncnt != 2) {
        ERR_raise_data(ERR_LIB_EVP, UCI_R_INVALID_PROVIDER_FUNCTIONS,
...
void UCI_SIGNATURE_free(UCI_SIGNATURE *signature)
{
    int i;
...
int UCI_SIGNATURE_up_ref(UCI_SIGNATURE *signature)
{
    int ref = 0;
...
OSSL_PROVIDER *UCI_SIGNATURE_get0_provider(const UCI_SIGNATURE *signature)
{
    return signature->prov;
}

UCI_SIGNATURE *UCI_SIGNATURE_fetch(OSSL_LIB_CTX *ctx, const char *algorithm,
                                   const char *properties)
{
    return uci_generic_fetch(ctx, OSSL_OP_SIGNATURE, algorithm, properties,
                             uci_signature_from_algorithm,
                             uci_signature_up_ref,
                             uci_signature_free);
}

UCI_SIGNATURE *uci_signature_fetch_from_prov(OSSL_PROVIDER *prov,
                                             const char *algorithm,
                                             const char *properties)
{
    return uci_generic_fetch_from_prov(prov, OSSL_OP_SIGNATURE,
...
int UCI_SIGNATURE_is_a(const UCI_SIGNATURE *signature, const char *name)
{
    return signature != NULL
           && uci_is_a(signature->prov, signature->name_id, NULL, name);
}

int uci_signature_get_number(const UCI_SIGNATURE *signature)
{
    return signature->name_id;
}

const char *UCI_SIGNATURE_get0_name(const UCI_SIGNATURE *signature)
{
    return signature->type_name;
}

const char *UCI_SIGNATURE_get0_description(const UCI_SIGNATURE *signature)
{
    return signature->description;
}

void UCI_SIGNATURE_do_all_provided(OSSL_LIB_CTX *libctx,
                                   void (*fn)(UCI_SIGNATURE *signature,
                                              void *arg),
                                   void *arg)
{
    uci_generic_do_all(libctx, OSSL_OP_SIGNATURE,
...
int UCI_SIGNATURE_names_do_all(const UCI_SIGNATURE *signature,
                               void (*fn)(const char *name, void *data),
                               void *data)
{
    if (signature->prov != NULL)
        return uci_names_do_all(signature->prov, signature->name_id, fn, data);

    return 1;
}

const OSSL_PARAM *UCI_SIGNATURE_gettable_ctx_params(const UCI_SIGNATURE *sig)
{
    void *provctx;
    ...
    provctx = ossl_provider_ctx(UCI_SIGNATURE_get0_provider(sig));
    return sig->gettable_ctx_params(NULL, provctx);
}

const OSSL_PARAM *UCI_SIGNATURE_settable_ctx_params(const UCI_SIGNATURE *sig)
{
    void *provctx;
...
static int uci_pkey_signature_init(UCI_PKEY_CTX *ctx, UCI_SIGNATURE *signature,
                                   int operation, const OSSL_PARAM params[])
{
    ...
    if (signature != NULL) {
         ...
         * try to export |ctx->pkey| to it (uci_pkey_export_to_provider()
         * is smart enough to only actually export it if |tmp_keymgmt|
         * is different from |ctx->pkey|'s keymgmt)
         */
        tmp_prov = UCI_SIGNATURE_get0_provider(signature);
        ...
        if (signature->query_key_types != NULL) {
            ...
            for (; *keytypes != NULL; keytypes++)
                if (UCI_PKEY_CTX_is_a(ctx, *keytypes))
                    break;
            if (*keytypes == NULL) {
                ERR_raise(ERR_LIB_EVP, UCI_R_SIGNATURE_TYPE_AND_KEY_TYPE_INCOMPATIBLE);
                return -2;
            }
        } else {
            /*
             ...
             */
            const char *keytype = UCI_KEYMGMT_get0_name(ctx->keymgmt);
            int ok = UCI_SIGNATURE_is_a(signature, keytype);
            ...
            if (!ok) {
                const char *signame
                    = uci_keymgmt_util_query_operation_name(ctx->keymgmt,
                                                            OSSL_OP_SIGNATURE);

                ok = UCI_SIGNATURE_is_a(signature, signame);
            }
        ...
        if (!UCI_SIGNATURE_up_ref(signature))
            return 0;
    } else {
        ...
        for (iter = 1; iter < 3 && provkey == NULL; iter++) {
            ...
            case 1:
                signature =
                    UCI_SIGNATURE_fetch(ctx->libctx, supported_sig, ctx->propquery);
                if (signature != NULL)
                    tmp_prov = UCI_SIGNATURE_get0_provider(signature);
                break;
            case 2:
                ...
                signature =
                    uci_signature_fetch_from_prov((OSSL_PROVIDER *)tmp_prov,
    ...
    switch (operation) {
    case UCI_PKEY_OP_SIGN:
        if (signature->sign_init == NULL) {
            ERR_raise_data(ERR_LIB_EVP, UCI_R_PROVIDER_SIGNATURE_NOT_SUPPORTED,
    ...
    case UCI_PKEY_OP_VERIFYMSG:
        ...
        break;
    case UCI_PKEY_OP_VERIFYRECOVER:
        if (signature->verify_recover_init == NULL) {
            ERR_raise_data(ERR_LIB_EVP, UCI_R_PROVIDER_SIGNATURE_NOT_SUPPORTED,
 ...
 legacy:
    ...
    if (ctx->pmeth == NULL
            || (operation == UCI_PKEY_OP_SIGN && ctx->pmeth->sign == NULL)
            || (operation == UCI_PKEY_OP_VERIFY && ctx->pmeth->verify == NULL)
            || (operation == UCI_PKEY_OP_VERIFYRECOVER
...
#ifndef FIPS_MODULE
    if (ret > 0)
        ret = uci_pkey_ctx_use_cached_data(ctx);
#endif
...
int UCI_PKEY_sign_init(UCI_PKEY_CTX *ctx)
{
    return uci_pkey_signature_init(ctx, NULL, UCI_PKEY_OP_SIGN, NULL);
}

int UCI_PKEY_sign_init_ex(UCI_PKEY_CTX *ctx, const OSSL_PARAM params[])
{
    return uci_pkey_signature_init(ctx, NULL, UCI_PKEY_OP_SIGN, params);
}

int UCI_PKEY_sign_init_ex2(UCI_PKEY_CTX *ctx,
                           UCI_SIGNATURE *algo, const OSSL_PARAM params[])
{
    return uci_pkey_signature_init(ctx, algo, UCI_PKEY_OP_SIGN, params);
}

int UCI_PKEY_sign_message_init(UCI_PKEY_CTX *ctx,
                               UCI_SIGNATURE *algo, const OSSL_PARAM params[])
{
    return uci_pkey_signature_init(ctx, algo, UCI_PKEY_OP_SIGNMSG, params);
}

int UCI_PKEY_sign_message_update(UCI_PKEY_CTX *ctx,
                                 const unsigned char *in, size_t inlen)
{
    UCI_SIGNATURE *signature;
    const char *desc;
    int ret;
    ...
    if (ctx->operation != UCI_PKEY_OP_SIGNMSG) {
    ...
    if (ret <= 0)
        ERR_raise_data(ERR_LIB_EVP, UCI_R_PROVIDER_SIGNATURE_FAILURE,
                       "%s sign_message_update:%s", signature->type_name, desc);
    return ret;
}

int UCI_PKEY_sign_message_final(UCI_PKEY_CTX *ctx,
...
int UCI_PKEY_sign(UCI_PKEY_CTX *ctx,
                  unsigned char *sig, size_t *siglen,
                  const unsigned char *tbs, size_t tbslen)
{
    ...
    if (ctx->operation != UCI_PKEY_OP_SIGN
        && ctx->operation != UCI_PKEY_OP_SIGNMSG) {
        ERR_raise(ERR_LIB_EVP, UCI_R_OPERATION_NOT_INITIALIZED);
        return -1;
    }
 ...
 legacy:
    ...
    M_check_autoarg(ctx, sig, siglen, UCI_F_UCI_PKEY_SIGN)
        return ctx->pmeth->sign(ctx, sig, siglen, tbs, tbslen);
}

int UCI_PKEY_verify_init(UCI_PKEY_CTX *ctx)
{
    return uci_pkey_signature_init(ctx, NULL, UCI_PKEY_OP_VERIFY, NULL);
}

int UCI_PKEY_verify_init_ex(UCI_PKEY_CTX *ctx, const OSSL_PARAM params[])
{
    return uci_pkey_signature_init(ctx, NULL, UCI_PKEY_OP_VERIFY, params);
}

int UCI_PKEY_verify_init_ex2(UCI_PKEY_CTX *ctx,
                             UCI_SIGNATURE *algo, const OSSL_PARAM params[])
{
    return uci_pkey_signature_init(ctx, algo, UCI_PKEY_OP_VERIFY, params);
}

int UCI_PKEY_verify_message_init(UCI_PKEY_CTX *ctx,
                                 UCI_SIGNATURE *algo, const OSSL_PARAM params[])
{
    return uci_pkey_signature_init(ctx, algo, UCI_PKEY_OP_VERIFYMSG, params);
}

int UCI_PKEY_CTX_set_signature(UCI_PKEY_CTX *ctx,
                               const unsigned char *sig, size_t siglen)
{
    ...
    *p = OSSL_PARAM_construct_end();
    ...
    return UCI_PKEY_CTX_set_params(ctx, sig_params);
}

int UCI_PKEY_verify_message_update(UCI_PKEY_CTX *ctx,
                                   const unsigned char *in, size_t inlen)
{
    UCI_SIGNATURE *signature;
    const char *desc;
    ...
    if (ctx->operation != UCI_PKEY_OP_VERIFYMSG) {
        ERR_raise(ERR_LIB_EVP, UCI_R_OPERATION_NOT_INITIALIZED);
        return -1;
    }
...
int UCI_PKEY_verify_message_final(UCI_PKEY_CTX *ctx)
{
    UCI_SIGNATURE *signature;
    const char *desc;
    int ret;
    ...
    /* The signature must have been set with UCI_PKEY_CTX_set_signature() */
...
int UCI_PKEY_verify(UCI_PKEY_CTX *ctx,
                    const unsigned char *sig, size_t siglen,
                    const unsigned char *tbs, size_t tbslen)
{
    UCI_SIGNATURE *signature;
    ...
    if (ctx->operation != UCI_PKEY_OP_VERIFY
        && ctx->operation != UCI_PKEY_OP_VERIFYMSG) {
        ERR_raise(ERR_LIB_EVP, UCI_R_OPERATION_NOT_INITIALIZED);
        return -1;
    }
...
int UCI_PKEY_verify_recover_init(UCI_PKEY_CTX *ctx)
{
    return uci_pkey_signature_init(ctx, NULL, UCI_PKEY_OP_VERIFYRECOVER, NULL);
}

int UCI_PKEY_verify_recover_init_ex(UCI_PKEY_CTX *ctx,
                                    const OSSL_PARAM params[])
{
    return uci_pkey_signature_init(ctx, NULL, UCI_PKEY_OP_VERIFYRECOVER, params);
}

int UCI_PKEY_verify_recover_init_ex2(UCI_PKEY_CTX *ctx,
                                     UCI_SIGNATURE *algo, const OSSL_PARAM params[])
{
    return uci_pkey_signature_init(ctx, algo, UCI_PKEY_OP_VERIFYRECOVER, params);
}

int UCI_PKEY_verify_recover(UCI_PKEY_CTX *ctx,
                            ...
                            const unsigned char *sig, size_t siglen)
{
    ...
    if (ctx->operation != UCI_PKEY_OP_VERIFYRECOVER) {
        ERR_raise(ERR_LIB_EVP, UCI_R_OPERATION_NOT_INITIALIZED);
        return -1;
    }
 ...
 legacy:
    ...
    M_check_autoarg(ctx, rout, routlen, UCI_F_UCI_PKEY_VERIFY_RECOVER)
