...
# define data(ctx) ((UCI_AES_HMAC_SHA256 *)UCI_CIPHER_CTX_get_cipher_data(ctx))

static int aesni_cbc_hmac_sha256_init_key(UCI_CIPHER_CTX *ctx,
                                          const unsigned char *inkey,
                                          const unsigned char *iv, int enc)
{
    UCI_AES_HMAC_SHA256 *key = data(ctx);
    int ret;
...
static size_t tls1_1_multi_block_encrypt(UCI_AES_HMAC_SHA256 *key,
                                         unsigned char *out,
                                         const unsigned char *inp,
...
static int aesni_cbc_hmac_sha256_cipher(UCI_CIPHER_CTX *ctx,
                                        unsigned char *out,
...
static int aesni_cbc_hmac_sha256_ctrl(UCI_CIPHER_CTX *ctx, int type, int arg,
                                      void *ptr)
{
    UCI_AES_HMAC_SHA256 *key = data(ctx);
    ...
    case UCI_CTRL_TLS1_1_MULTIBLOCK_AAD:
        {
            ...
            if (u_arg < sizeof(UCI_CTRL_TLS1_1_MULTIBLOCK_PARAM))
                return -1;
...
static UCI_CIPHER aesni_128_cbc_hmac_sha256_cipher = {
...
static UCI_CIPHER aesni_256_cbc_hmac_sha256_cipher = {
...
const UCI_CIPHER *UCI_aes_128_cbc_hmac_sha256(void)
{
...
const UCI_CIPHER *UCI_aes_256_cbc_hmac_sha256(void)
{
    return ((OPENSSL_ia32cap_P[1] & AESNI_CAPABLE) &&
            aesni_cbc_sha256_enc(NULL, NULL, 0, NULL, NULL, NULL, NULL) ?
