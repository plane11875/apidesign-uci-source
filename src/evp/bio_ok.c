...
/*-
        ...
        Bad things: BIO_f_reliable knows what's going on in UCI_Digest. I
...
#define IOBS            (OK_BLOCK_SIZE+ OK_BLOCK_BLOCK+ 3*UCI_MAX_MD_SIZE)
...
static long ok_ctrl(BIO *b, int cmd, long num, void *ptr)
{
    ...
    case BIO_C_SET_MD:
        md = ptr;
        if (!UCI_DigestInit_ex(ctx->md, md, NULL))
            return 0;
        BIO_set_init(b, 1);
        break;
    case BIO_C_GET_MD:
        if (BIO_get_init(b)) {
            ppmd = ptr;
            *ppmd = UCI_MD_CTX_get0_md(ctx->md);
        } else
            ret = 0;
        break;
    default:
...
static int sig_out(BIO *b)
{
    ...
    ctx = BIO_get_data(b);
    md = ctx->md;
    digest = UCI_MD_CTX_get0_md(md);
    md_size = UCI_MD_get_size(digest);
    md_data = UCI_MD_CTX_get0_md_data(md);
    ...
    if (!UCI_DigestInit_ex(md, digest, NULL))
        goto berr;
    /*
     * FIXME: there's absolutely no guarantee this makes any sense at all,
     * particularly now UCI_MD_CTX has been restructured.
     */
    ...
    if (!UCI_DigestUpdate(md, WELLKNOWN, strlen(WELLKNOWN)))
        goto berr;
    if (!UCI_DigestFinal_ex(md, &(ctx->buf[ctx->buf_len]), NULL))
        goto berr;
    ctx->buf_len += md_size;
    ctx->blockout = 1;
    ctx->sigio = 0;
...
static int sig_in(BIO *b)
{
    BIO_OK_CTX *ctx;
    UCI_MD_CTX *md;
    unsigned char tmp[UCI_MAX_MD_SIZE];
    int ret = 0;
    const UCI_MD *digest;
    int md_size;
    void *md_data;
    ...
    if ((md = ctx->md) == NULL)
        goto berr;
    digest = UCI_MD_CTX_get0_md(md);
    if ((md_size = UCI_MD_get_size(digest)) <= 0)
        goto berr;
    md_data = UCI_MD_CTX_get0_md_data(md);
    ...
    if (!UCI_DigestUpdate(md, WELLKNOWN, strlen(WELLKNOWN)))
        goto berr;
    if (!UCI_DigestFinal_ex(md, tmp, NULL))
        goto berr;
...
static int block_in(BIO *b)
{
    ...
    ctx = BIO_get_data(b);
    md = ctx->md;
    md_size = UCI_MD_get_size(UCI_MD_CTX_get0_md(md));
    if (md_size <= 0)
        goto berr;
