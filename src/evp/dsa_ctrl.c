...
static int dsa_paramgen_check(UCI_PKEY_CTX *ctx)
{
    ...
    if (ctx->pmeth != NULL && ctx->pmeth->pkey_id != UCI_PKEY_DSA)
        return -1;
    return 1;
}

int UCI_PKEY_CTX_set_dsa_paramgen_type(UCI_PKEY_CTX *ctx, const char *name)
{
    int ret;
...
int UCI_PKEY_CTX_set_dsa_paramgen_gindex(UCI_PKEY_CTX *ctx, int gindex)
{
    int ret;
    OSSL_PARAM params[2], *p = params;
...
int UCI_PKEY_CTX_set_dsa_paramgen_seed(UCI_PKEY_CTX *ctx,
...
int UCI_PKEY_CTX_set_dsa_paramgen_bits(UCI_PKEY_CTX *ctx, int nbits)
{
    int ret;
    OSSL_PARAM params[2], *p = params;
    size_t bits = nbits;
...
int UCI_PKEY_CTX_set_dsa_paramgen_q_bits(UCI_PKEY_CTX *ctx, int qbits)
{
    int ret;
    OSSL_PARAM params[2], *p = params;
    size_t bits2 = qbits;
...
int UCI_PKEY_CTX_set_dsa_paramgen_md_props(UCI_PKEY_CTX *ctx,
                                           const char *md_name,
...
int UCI_PKEY_CTX_set_dsa_paramgen_md(UCI_PKEY_CTX *ctx, const UCI_MD *md)
{
    return UCI_PKEY_CTX_ctrl(ctx, UCI_PKEY_DSA, UCI_PKEY_OP_PARAMGEN,
                             UCI_PKEY_CTRL_DSA_PARAMGEN_MD, 0, (void *)(md));
}
#endif
