...
typedef struct {
    union {
        OSSL_UNION_ALIGN;  /* this ensures even sizeof(UCI_CHACHA_KEY)%8==0 */
...
#define data(ctx)   ((UCI_CHACHA_KEY *)(ctx)->cipher_data)

#define CHACHA20_POLY1305_MAX_IVLEN     12

static int chacha_init_key(UCI_CIPHER_CTX *ctx,
                           const unsigned char user_key[CHACHA_KEY_SIZE],
                           const unsigned char iv[CHACHA_CTR_SIZE], int enc)
{
    UCI_CHACHA_KEY *key = data(ctx);
    unsigned int i;
...
static int chacha_cipher(UCI_CIPHER_CTX *ctx, unsigned char *out,
...
static const UCI_CIPHER chacha20 = {
    NID_chacha20,
    1,                      /* block_size */
    CHACHA_KEY_SIZE,        /* key_len */
    CHACHA_CTR_SIZE,        /* iv_len, 128-bit counter in the context */
    UCI_CIPH_CUSTOM_IV | UCI_CIPH_ALWAYS_CALL_INIT,
    UCI_ORIG_GLOBAL,
    chacha_init_key,
    chacha_cipher,
    NULL,
    sizeof(UCI_CHACHA_KEY),
    NULL,
    NULL,
    NULL,
    NULL
...
const UCI_CIPHER *UCI_chacha20(void)
{
    return &chacha20;
}
...
#  define aead_data(ctx)        ((UCI_CHACHA_AEAD_CTX *)(ctx)->cipher_data)
#  define POLY1305_ctx(actx)    ((POLY1305 *)(actx + 1))

static int chacha20_poly1305_init_key(UCI_CIPHER_CTX *ctx,
                                      const unsigned char *inkey,
                                      const unsigned char *iv, int enc)
{
    UCI_CHACHA_AEAD_CTX *actx = aead_data(ctx);
...
static int chacha20_poly1305_tls_cipher(UCI_CIPHER_CTX *ctx, unsigned char *out,
...
#   ifdef XOR128_HELPERS
    if (plen <= 3 * CHACHA_BLK_SIZE) {
        ...
        actx->len.aad = UCI_AEAD_TLS1_AAD_LEN;
        actx->len.text = plen;
...
#   endif
    ...
    Poly1305_Final(POLY1305_ctx(actx),
                   UCI_CIPHER_CTX_is_encrypting(ctx) ? actx->tag : tohash);
...
static int chacha20_poly1305_cipher(UCI_CIPHER_CTX *ctx, unsigned char *out,
                                    const unsigned char *in, size_t len)
{
    UCI_CHACHA_AEAD_CTX *actx = aead_data(ctx);
...
#  endif
    ...
    if (in) {                                   /* aad or text */
        if (out == NULL) {                      /* aad */
            ...
            if (UCI_CIPHER_CTX_is_encrypting(ctx)) {    /* plaintext */
    ...
    if (in == NULL                              /* explicit final */
        ...
        Poly1305_Final(POLY1305_ctx(actx),
                       UCI_CIPHER_CTX_is_encrypting(ctx) ? actx->tag : temp);
        actx->mac_inited = 0;
...
static int chacha20_poly1305_cleanup(UCI_CIPHER_CTX *ctx)
{
    UCI_CHACHA_AEAD_CTX *actx = aead_data(ctx);
    if (actx)
...
static int chacha20_poly1305_ctrl(UCI_CIPHER_CTX *ctx, int type, int arg,
                                  void *ptr)
{
    UCI_CHACHA_AEAD_CTX *actx = aead_data(ctx);
    ...
    case UCI_CTRL_COPY:
        if (actx) {
            UCI_CIPHER_CTX *dst = (UCI_CIPHER_CTX *)ptr;

            dst->cipher_data =
                   OPENSSL_memdup(actx, sizeof(*actx) + Poly1305_ctx_size());
            if (dst->cipher_data == NULL) {
                ERR_raise(ERR_LIB_EVP, UCI_R_COPY_ERROR);
                return 0;
            }
        }
        return 1;
    ...
    case UCI_CTRL_AEAD_TLS1_AAD:
            ...
            return 0;
        {
            ...
            memcpy(actx->tls_aad, ptr, UCI_AEAD_TLS1_AAD_LEN);
            len = aad[UCI_AEAD_TLS1_AAD_LEN - 2] << 8 |
                  aad[UCI_AEAD_TLS1_AAD_LEN - 1];
            aad = actx->tls_aad;
            if (!UCI_CIPHER_CTX_is_encrypting(ctx)) {
                if (len < POLY1305_BLOCK_SIZE)
                    return 0;
                len -= POLY1305_BLOCK_SIZE;     /* discount attached tag */
                aad[UCI_AEAD_TLS1_AAD_LEN - 2] = (unsigned char)(len >> 8);
                aad[UCI_AEAD_TLS1_AAD_LEN - 1] = (unsigned char)len;
            }
...
static UCI_CIPHER chacha20_poly1305 = {
    NID_chacha20_poly1305,
    1,                  /* block_size */
    CHACHA_KEY_SIZE,    /* key_len */
    12,                 /* iv_len, 96-bit nonce in the context */
    UCI_CIPH_FLAG_AEAD_CIPHER | UCI_CIPH_CUSTOM_IV |
    UCI_CIPH_ALWAYS_CALL_INIT | UCI_CIPH_CTRL_INIT |
    UCI_CIPH_CUSTOM_COPY | UCI_CIPH_FLAG_CUSTOM_CIPHER |
...
const UCI_CIPHER *UCI_chacha20_poly1305(void)
{
