...
#include <openssl/asn1.h>        /* uci_local.h needs it */
#include <openssl/safestack.h>   /* uci_local.h needs it */
#include "crypto/evp.h"    /* uci_local.h needs it */
#include "uci_local.h"

/*
 * UCI_CTRL_RET_UNSUPPORTED = -1 is the returned value from any ctrl function
...
{
    ERR_raise(ERR_LIB_EVP, UCI_R_CANNOT_GET_PARAMETERS);
}

static void seterr(void)
{
    ERR_raise(ERR_LIB_EVP, UCI_R_CANNOT_SET_PARAMETERS);
}

PARAM_FUNCTIONS(UCI_CIPHER,
                uci_do_ciph_getparams, get_params,
                uci_do_ciph_ctx_getparams, get_ctx_params,
                uci_do_ciph_ctx_setparams, set_ctx_params)

PARAM_FUNCTIONS(UCI_MD,
                uci_do_md_getparams, get_params,
                uci_do_md_ctx_getparams, get_ctx_params,
                uci_do_md_ctx_setparams, set_ctx_params)
