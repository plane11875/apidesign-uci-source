...
static void uci_asym_cipher_free(void *data)
{
    UCI_ASYM_CIPHER_free(data);
}

static int uci_asym_cipher_up_ref(void *data)
{
    return UCI_ASYM_CIPHER_up_ref(data);
}

static int uci_pkey_asym_cipher_init(UCI_PKEY_CTX *ctx, int operation,
                                     const OSSL_PARAM params[])
{
    int ret = 0;
    void *provkey = NULL;
    UCI_ASYM_CIPHER *cipher = NULL;
    const char *desc;
    ...
    for (iter = 1, provkey = NULL; iter < 3 && provkey == NULL; iter++) {
        UCI_KEYMGMT *tmp_keymgmt_tofree;
         ...
         * iteration we're on.
         */
        UCI_ASYM_CIPHER_free(cipher);
        UCI_KEYMGMT_free(tmp_keymgmt);

        switch (iter) {
        case 1:
            cipher = UCI_ASYM_CIPHER_fetch(ctx->libctx, supported_ciph,
                                           ctx->propquery);
            if (cipher != NULL)
                tmp_prov = UCI_ASYM_CIPHER_get0_provider(cipher);
            break;
        case 2:
            tmp_prov = UCI_KEYMGMT_get0_provider(ctx->keymgmt);
            cipher =
                uci_asym_cipher_fetch_from_prov((OSSL_PROVIDER *)tmp_prov,
    ...
    switch (operation) {
    case UCI_PKEY_OP_ENCRYPT:
        if (cipher->encrypt_init == NULL) {
            ERR_raise_data(ERR_LIB_EVP, UCI_R_PROVIDER_ASYM_CIPHER_NOT_SUPPORTED,
...
int UCI_PKEY_encrypt_init(UCI_PKEY_CTX *ctx)
{
    return uci_pkey_asym_cipher_init(ctx, UCI_PKEY_OP_ENCRYPT, NULL);
}

int UCI_PKEY_encrypt_init_ex(UCI_PKEY_CTX *ctx, const OSSL_PARAM params[])
{
    return uci_pkey_asym_cipher_init(ctx, UCI_PKEY_OP_ENCRYPT, params);
}

int UCI_PKEY_encrypt(UCI_PKEY_CTX *ctx,
                     unsigned char *out, size_t *outlen,
                     const unsigned char *in, size_t inlen)
{
    UCI_ASYM_CIPHER *cipher;
    const char *desc;
    int ret;
    ...
    if (ctx->operation != UCI_PKEY_OP_ENCRYPT) {
    ...
    if (ret <= 0 && ERR_count_to_mark() == 0)
        ERR_raise_data(ERR_LIB_EVP, UCI_R_PROVIDER_ASYM_CIPHER_FAILURE,
 ...
 legacy:
    ...
    M_check_autoarg(ctx, out, outlen, UCI_F_UCI_PKEY_ENCRYPT)
        return ctx->pmeth->encrypt(ctx, out, outlen, in, inlen);
}

int UCI_PKEY_decrypt_init(UCI_PKEY_CTX *ctx)
{
    return uci_pkey_asym_cipher_init(ctx, UCI_PKEY_OP_DECRYPT, NULL);
}

int UCI_PKEY_decrypt_init_ex(UCI_PKEY_CTX *ctx, const OSSL_PARAM params[])
{
    return uci_pkey_asym_cipher_init(ctx, UCI_PKEY_OP_DECRYPT, params);
}

int UCI_PKEY_decrypt(UCI_PKEY_CTX *ctx,
                     unsigned char *out, size_t *outlen,
                     const unsigned char *in, size_t inlen)
{
    UCI_ASYM_CIPHER *cipher;
    ...
    if (ctx->operation != UCI_PKEY_OP_DECRYPT) {
        ERR_raise(ERR_LIB_EVP, UCI_R_OPERATION_NOT_INITIALIZED);
        return -1;
    }
 ...
 legacy:
    ...
    M_check_autoarg(ctx, out, outlen, UCI_F_UCI_PKEY_DECRYPT)
        return ctx->pmeth->decrypt(ctx, out, outlen, in, inlen);
}

/* decrypt to new buffer of dynamic size, checking any pre-determined size */
int uci_pkey_decrypt_alloc(UCI_PKEY_CTX *ctx, unsigned char **outp,
                           size_t *outlenp, size_t expected_outlen,
                           const unsigned char *in, size_t inlen)
{
    if (UCI_PKEY_decrypt(ctx, NULL, outlenp, in, inlen) <= 0
            || (*outp = OPENSSL_malloc(*outlenp)) == NULL)
        return -1;
    if (UCI_PKEY_decrypt(ctx, *outp, outlenp, in, inlen) <= 0
            || *outlenp == 0
...
static UCI_ASYM_CIPHER *uci_asym_cipher_new(OSSL_PROVIDER *prov)
{
    UCI_ASYM_CIPHER *cipher = OPENSSL_zalloc(sizeof(UCI_ASYM_CIPHER));
...
static void *uci_asym_cipher_from_algorithm(int name_id,
                                            ...
                                            OSSL_PROVIDER *prov)
{
    ...
    if ((cipher = uci_asym_cipher_new(prov)) == NULL) {
        ERR_raise(ERR_LIB_EVP, ERR_R_UCI_LIB);
        goto err;
    }
...
void UCI_ASYM_CIPHER_free(UCI_ASYM_CIPHER *cipher)
{
    int i;
...
int UCI_ASYM_CIPHER_up_ref(UCI_ASYM_CIPHER *cipher)
{
    int ref = 0;
...
OSSL_PROVIDER *UCI_ASYM_CIPHER_get0_provider(const UCI_ASYM_CIPHER *cipher)
{
    return cipher->prov;
}

UCI_ASYM_CIPHER *UCI_ASYM_CIPHER_fetch(OSSL_LIB_CTX *ctx, const char *algorithm,
                                       const char *properties)
{
    return uci_generic_fetch(ctx, OSSL_OP_ASYM_CIPHER, algorithm, properties,
                             uci_asym_cipher_from_algorithm,
                             uci_asym_cipher_up_ref,
                             uci_asym_cipher_free);
}

UCI_ASYM_CIPHER *uci_asym_cipher_fetch_from_prov(OSSL_PROVIDER *prov,
                                                 const char *algorithm,
                                                 const char *properties)
{
    return uci_generic_fetch_from_prov(prov, OSSL_OP_ASYM_CIPHER,
...
int UCI_ASYM_CIPHER_is_a(const UCI_ASYM_CIPHER *cipher, const char *name)
{
    return uci_is_a(cipher->prov, cipher->name_id, NULL, name);
}

int uci_asym_cipher_get_number(const UCI_ASYM_CIPHER *cipher)
{
    return cipher->name_id;
}

const char *UCI_ASYM_CIPHER_get0_name(const UCI_ASYM_CIPHER *cipher)
{
    return cipher->type_name;
}

const char *UCI_ASYM_CIPHER_get0_description(const UCI_ASYM_CIPHER *cipher)
{
    return cipher->description;
}

void UCI_ASYM_CIPHER_do_all_provided(OSSL_LIB_CTX *libctx,
                                     void (*fn)(UCI_ASYM_CIPHER *cipher,
                                                void *arg),
                                     void *arg)
{
    uci_generic_do_all(libctx, OSSL_OP_ASYM_CIPHER,
...
int UCI_ASYM_CIPHER_names_do_all(const UCI_ASYM_CIPHER *cipher,
...
const OSSL_PARAM *UCI_ASYM_CIPHER_gettable_ctx_params(const UCI_ASYM_CIPHER *cip)
{
    void *provctx;
    ...
    provctx = ossl_provider_ctx(UCI_ASYM_CIPHER_get0_provider(cip));
    return cip->gettable_ctx_params(NULL, provctx);
}

const OSSL_PARAM *UCI_ASYM_CIPHER_settable_ctx_params(const UCI_ASYM_CIPHER *cip)
{
    void *provctx;
