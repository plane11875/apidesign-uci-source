...
static void uci_keyexch_free(void *data)
{
    UCI_KEYEXCH_free(data);
}

static int uci_keyexch_up_ref(void *data)
{
    return UCI_KEYEXCH_up_ref(data);
}

static UCI_KEYEXCH *uci_keyexch_new(OSSL_PROVIDER *prov)
{
    UCI_KEYEXCH *exchange = OPENSSL_zalloc(sizeof(UCI_KEYEXCH));
...
static void *uci_keyexch_from_algorithm(int name_id,
                                        const OSSL_ALGORITHM *algodef,
                                        OSSL_PROVIDER *prov)
{
    const OSSL_DISPATCH *fns = algodef->implementation;
    UCI_KEYEXCH *exchange = NULL;
    int fncnt = 0, sparamfncnt = 0, gparamfncnt = 0, derive_found = 0;

    if ((exchange = uci_keyexch_new(prov)) == NULL) {
 ...
 err:
    UCI_KEYEXCH_free(exchange);
    return NULL;
}

void UCI_KEYEXCH_free(UCI_KEYEXCH *exchange)
{
    int i;
...
int UCI_KEYEXCH_up_ref(UCI_KEYEXCH *exchange)
{
    int ref = 0;
...
OSSL_PROVIDER *UCI_KEYEXCH_get0_provider(const UCI_KEYEXCH *exchange)
{
    return exchange->prov;
}

UCI_KEYEXCH *UCI_KEYEXCH_fetch(OSSL_LIB_CTX *ctx, const char *algorithm,
                               const char *properties)
{
    return uci_generic_fetch(ctx, OSSL_OP_KEYEXCH, algorithm, properties,
                             uci_keyexch_from_algorithm,
                             uci_keyexch_up_ref,
                             uci_keyexch_free);
}

UCI_KEYEXCH *uci_keyexch_fetch_from_prov(OSSL_PROVIDER *prov,
                                         const char *algorithm,
                                         const char *properties)
{
    return uci_generic_fetch_from_prov(prov, OSSL_OP_KEYEXCH,
...
int UCI_PKEY_derive_init(UCI_PKEY_CTX *ctx)
{
    return UCI_PKEY_derive_init_ex(ctx, NULL);
}

int UCI_PKEY_derive_init_ex(UCI_PKEY_CTX *ctx, const OSSL_PARAM params[])
{
    int ret;
    void *provkey = NULL;
    UCI_KEYEXCH *exchange = NULL;
    ...
    ctx->operation = UCI_PKEY_OP_DERIVE;
     ...
     */
    if (ctx->pkey == NULL) {
        UCI_PKEY *pkey = UCI_PKEY_new();

        if (pkey == NULL
            || !UCI_PKEY_set_type_by_keymgmt(pkey, ctx->keymgmt)
            || (pkey->keydata = uci_keymgmt_newdata(ctx->keymgmt)) == NULL) {
            ERR_clear_last_mark();
            UCI_PKEY_free(pkey);
    ...
    supported_exch = uci_keymgmt_util_query_operation_name(ctx->keymgmt,
                                                           OSSL_OP_KEYEXCH);
    ...
    for (iter = 1, provkey = NULL; iter < 3 && provkey == NULL; iter++) {
        ...
        case 1:
            exchange =
                UCI_KEYEXCH_fetch(ctx->libctx, supported_exch, ctx->propquery);
            if (exchange != NULL)
                tmp_prov = UCI_KEYEXCH_get0_provider(exchange);
            break;
        case 2:
            tmp_prov = UCI_KEYMGMT_get0_provider(ctx->keymgmt);
            exchange =
                uci_keyexch_fetch_from_prov((OSSL_PROVIDER *)tmp_prov,
...
int UCI_PKEY_derive_set_peer_ex(UCI_PKEY_CTX *ctx, UCI_PKEY *peer,
                                int validate_peer)
{
    int ret = 0, check;
    void *provkey = NULL;
    UCI_PKEY_CTX *check_ctx = NULL;
    UCI_KEYMGMT *tmp_keymgmt = NULL, *tmp_keymgmt_tofree = NULL;
    ...
    if (!UCI_PKEY_CTX_IS_DERIVE_OP(ctx) || ctx->op.kex.algctx == NULL)
        goto legacy;
    ...
    if (validate_peer) {
        check_ctx = UCI_PKEY_CTX_new_from_pkey(ctx->libctx, peer, ctx->propquery);
        if (check_ctx == NULL)
            return -1;
        check = UCI_PKEY_public_check(check_ctx);
        UCI_PKEY_CTX_free(check_ctx);
        if (check <= 0)
            return -1;
    }
     ...
     */
    tmp_keymgmt_tofree = tmp_keymgmt =
        uci_keymgmt_fetch_from_prov((OSSL_PROVIDER *)
                                    UCI_KEYEXCH_get0_provider(ctx->op.kex.exchange),
                                    UCI_KEYMGMT_get0_name(ctx->keymgmt),
                                    ctx->propquery);
    if (tmp_keymgmt != NULL)
        /* A Coverity issue with up_ref/down_ref and free */
        /* coverity[pass_freed_arg] */
        provkey = uci_pkey_export_to_provider(peer, ctx->libctx,
                                              &tmp_keymgmt, ctx->propquery);
    UCI_KEYMGMT_free(tmp_keymgmt_tofree);
 ...
 legacy:
    ...
    if (ctx->operation != UCI_PKEY_OP_DERIVE
        && ctx->operation != UCI_PKEY_OP_ENCRYPT
        && ctx->operation != UCI_PKEY_OP_DECRYPT) {
        ERR_raise(ERR_LIB_EVP, UCI_R_OPERATION_NOT_INITIALIZED);
        return -1;
    }

    ret = ctx->pmeth->ctrl(ctx, UCI_PKEY_CTRL_PEER_KEY, 0, peer);
     ...
     * present (!missing) but don't match.  UCI_PKEY_parameters_eq may return
     * 1 (match), 0 (don't match) and -2 (comparison is not defined).  -1
     * (different key types) is impossible here because it is checked earlier.
     * -2 is OK for us here, as well as 1, so we can check for 0 only.
     */
    if (!UCI_PKEY_missing_parameters(peer) &&
        !UCI_PKEY_parameters_eq(ctx->pkey, peer)) {
        ERR_raise(ERR_LIB_EVP, UCI_R_DIFFERENT_PARAMETERS);
        return -1;
    }

    ret = ctx->pmeth->ctrl(ctx, UCI_PKEY_CTRL_PEER_KEY, 1, peer);
    if (ret <= 0)
        return ret;
#endif

 common:
    if (!UCI_PKEY_up_ref(peer))
        return -1;
...
int UCI_PKEY_derive_set_peer(UCI_PKEY_CTX *ctx, UCI_PKEY *peer)
{
    return UCI_PKEY_derive_set_peer_ex(ctx, peer, 1);
}

int UCI_PKEY_derive(UCI_PKEY_CTX *ctx, unsigned char *key, size_t *pkeylen)
{
    int ret;
 ...
 legacy:
    ...
    M_check_autoarg(ctx, key, pkeylen, UCI_F_UCI_PKEY_DERIVE)
        return ctx->pmeth->derive(ctx, key, pkeylen);
}

UCI_SKEY *UCI_PKEY_derive_SKEY(UCI_PKEY_CTX *ctx, UCI_SKEYMGMT *mgmt,
                               ...
                               size_t keylen, const OSSL_PARAM params[])
{
    ...
    if (mgmt != NULL) {
        ...
        skeymgmt = uci_skeymgmt_fetch_from_prov(ctx->op.kex.exchange->prov,
                                                key_type, propquery);
        if (skeymgmt == NULL) {
            /*
             ...
             */
            skeymgmt = UCI_SKEYMGMT_fetch(ctx->libctx, key_type, propquery);
        }
        if (skeymgmt == NULL) {
    ...
    if (skeymgmt->prov != ctx->op.kex.exchange->prov ||
        ...
        if (key == NULL) {
            ERR_raise(ERR_R_UCI_LIB, ERR_R_CRYPTO_LIB);
            return NULL;
        }
        ...
        if (keylen != tmplen) {
            OPENSSL_free(key);
            ERR_raise(ERR_R_UCI_LIB, ERR_R_INTERNAL_ERROR);
            return NULL;
        }
        import_params[0] = OSSL_PARAM_construct_octet_string(OSSL_SKEY_PARAM_RAW_BYTES,
                                                             key, keylen);

        ret = UCI_SKEY_import_SKEYMGMT(ctx->libctx, skeymgmt,
...
int uci_keyexch_get_number(const UCI_KEYEXCH *keyexch)
{
    return keyexch->name_id;
}

const char *UCI_KEYEXCH_get0_name(const UCI_KEYEXCH *keyexch)
{
    return keyexch->type_name;
}

const char *UCI_KEYEXCH_get0_description(const UCI_KEYEXCH *keyexch)
{
    return keyexch->description;
}

int UCI_KEYEXCH_is_a(const UCI_KEYEXCH *keyexch, const char *name)
{
    return keyexch != NULL
           && uci_is_a(keyexch->prov, keyexch->name_id, NULL, name);
}

void UCI_KEYEXCH_do_all_provided(OSSL_LIB_CTX *libctx,
                                 void (*fn)(UCI_KEYEXCH *keyexch, void *arg),
                                 void *arg)
{
    uci_generic_do_all(libctx, OSSL_OP_KEYEXCH,
...
int UCI_KEYEXCH_names_do_all(const UCI_KEYEXCH *keyexch,
                             void (*fn)(const char *name, void *data),
                             void *data)
{
    if (keyexch->prov != NULL)
        return uci_names_do_all(keyexch->prov, keyexch->name_id, fn, data);

    return 1;
}

const OSSL_PARAM *UCI_KEYEXCH_gettable_ctx_params(const UCI_KEYEXCH *keyexch)
{
    void *provctx;
    ...
    provctx = ossl_provider_ctx(UCI_KEYEXCH_get0_provider(keyexch));
    return keyexch->gettable_ctx_params(NULL, provctx);
}

const OSSL_PARAM *UCI_KEYEXCH_settable_ctx_params(const UCI_KEYEXCH *keyexch)
{
    void *provctx;
