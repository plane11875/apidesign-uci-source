 ...
 * configured.  When we drop #legacy UCI_PKEYs, this source should be
...
/* The following includes get us all the UCI_PKEY_CTRL macros */
 ...
 *                              from an UCI_PKEY payload getter / setter,
 ...
 *                              from UCI_PKEY_CTX_ctrl_str(), to clean up
 *                              what the fixup_args function has done, if
 *                              needed.
 *
 * PRE_PARAMS_TO_CTRL           The fixup_args function has been called
 * POST_PARAMS_TO_CTRL          from UCI_PKEY_CTX_get_params() or
 *                              UCI_PKEY_CTX_set_params(), to help with
 *                              translating the OSSL_PARAM data to the
 *                              corresponding UCI_PKEY_CTX_ctrl() arguments
 ...
 *                              from UCI_PKEY_CTX_get_params() or
 *                              UCI_PKEY_CTX_set_params(), to clean up what
 *                              the fixup_args function has done, if needed.
 */
enum state {
    PKEY,
...
struct translation_ctx_st {
    /*
     * The UCI_PKEY_CTX, for calls on that structure, to be pilfered for data
     * as necessary.
     */
    UCI_PKEY_CTX *pctx;
    /*
...
struct translation_st {
     ...
     */
    int keytype1;    /* The UCI_PKEY_XXX type, i.e. NIDs. #legacy */
    int keytype2;    /* Another UCI_PKEY_XXX type, used for aliases */
     ...
     */
    int ctrl_num;            /* UCI_PKEY_CTRL_xxx */
                              ...
                              struct translation_ctx_st *ctx)
{
    ...
    case PRE_CTRL_TO_PARAMS:
        ...
        if (translation->optype != 0) {
            if ((UCI_PKEY_CTX_IS_SIGNATURE_OP(ctx->pctx)
                 && ctx->pctx->op.sig.algctx == NULL)
                || (UCI_PKEY_CTX_IS_DERIVE_OP(ctx->pctx)
                    && ctx->pctx->op.kex.algctx == NULL)
                || (UCI_PKEY_CTX_IS_ASYM_CIPHER_OP(ctx->pctx)
                    && ctx->pctx->op.ciph.algctx == NULL)
                || (UCI_PKEY_CTX_IS_KEM_OP(ctx->pctx)
                    && ctx->pctx->op.encap.algctx == NULL)
                /*
                 * The following may be unnecessary, but we have them
                 * for good measure...
                 */
                || (UCI_PKEY_CTX_IS_GEN_OP(ctx->pctx)
                    && ctx->pctx->op.keymgmt.genctx == NULL)
                || (UCI_PKEY_CTX_IS_FROMDATA_OP(ctx->pctx)
                    && ctx->pctx->op.keymgmt.genctx == NULL)) {
                ERR_raise(ERR_LIB_EVP, UCI_R_COMMAND_NOT_SUPPORTED);
    ...
    case POST_CTRL_TO_PARAMS:
        /*
         * Because UCI_PKEY_CTX_ctrl() returns the length of certain objects
    ...
    case PRE_CTRL_STR_TO_PARAMS:
        {
            ...
            if (ctx->action_type != OSSL_ACTION_SET) {
                ERR_raise_data(ERR_LIB_EVP, UCI_R_COMMAND_NOT_SUPPORTED,
            ...
            settable = UCI_PKEY_CTX_settable_params(ctx->pctx);
     ...
     * the UCI_PKEY_CTX_ctrl() return value (passed as |p1|) and |p2| back
     * to |*params|.
     *
...
/*
 * fix_cipher_md fixes up an UCI_CIPHER / UCI_MD to its name on OSSL_ACTION_SET,
 * and cipher / md name to UCI_MD on OSSL_ACTION_GET.
 */
static const char *get_cipher_name(void *cipher)
{
    return UCI_CIPHER_get0_name(cipher);
}

static const char *get_md_name(void *md)
{
    return UCI_MD_get0_name(md);
}

static const void *get_cipher_by_name(OSSL_LIB_CTX *libctx, const char *name)
{
    return uci_get_cipherbyname_ex(libctx, name);
}

static const void *get_md_by_name(OSSL_LIB_CTX *libctx, const char *name)
{
    return uci_get_digestbyname_ex(libctx, name);
}
                                                         ...
                                                         const char *name))
{
    ...
    if (state == PRE_CTRL_TO_PARAMS && ctx->action_type == OSSL_ACTION_GET) {
        /*
         * |ctx->p2| contains the address to an UCI_CIPHER or UCI_MD pointer
...
{
    /*
     * The UCI_PKEY_CTRL_DH_KDF_TYPE ctrl command is a bit special, in
...
/* UCI_PKEY_CTRL_DH_KDF_TYPE */
static int fix_dh_kdf_type(enum state state,
                           const struct translation_st *translation,
                           struct translation_ctx_st *ctx)
{
    static const struct kdf_type_map_st kdf_type_map[] = {
        { UCI_PKEY_DH_KDF_NONE, "" },
        { UCI_PKEY_DH_KDF_X9_42, OSSL_KDF_NAME_X942KDF_ASN1 },
        { 0, NULL }
    };
...
/* UCI_PKEY_CTRL_EC_KDF_TYPE */
static int fix_ec_kdf_type(enum state state,
                           const struct translation_st *translation,
                           struct translation_ctx_st *ctx)
{
    static const struct kdf_type_map_st kdf_type_map[] = {
        { UCI_PKEY_ECDH_KDF_NONE, "" },
        { UCI_PKEY_ECDH_KDF_X9_63, OSSL_KDF_NAME_X963KDF },
        { 0, NULL }
    };
...
/* UCI_PKEY_CTRL_DH_KDF_OID, UCI_PKEY_CTRL_GET_DH_KDF_OID, ...??? */
...
/* UCI_PKEY_CTRL_DH_PARAMGEN_TYPE */
...
/* UCI_PKEY_CTRL_EC_ECDH_COFACTOR */
static int fix_ecdh_cofactor(enum state state,
                             const struct translation_st *translation,
                             struct translation_ctx_st *ctx)
{
    /*
     * The UCI_PKEY_CTRL_EC_ECDH_COFACTOR ctrl command is a bit special, in
     * that it's used both for setting a value, and for getting it, all
    ...
    if (state == PRE_CTRL_TO_PARAMS) {
         ...
         * uci_pkey_ctrl_to_params() takes it from the translation item.
         */
        if (!ossl_assert(ctx->action_type == OSSL_ACTION_NONE))
            return 0;
...
/* UCI_PKEY_CTRL_RSA_PADDING, UCI_PKEY_CTRL_GET_RSA_PADDING */
                                ...
                                struct translation_ctx_st *ctx)
{
    ...
    if (state == PRE_CTRL_TO_PARAMS && ctx->action_type == OSSL_ACTION_GET) {
        /*
         * UCI_PKEY_CTRL_GET_RSA_PADDING returns the padding mode in the
         ...
         * We only came here because someone called UCI_PKEY_CTX_ctrl(),
        ...
        /*
         * The UCI_PKEY_CTX_get_params() caller may have asked for a utf8
    ...
    if ((ctx->action_type == OSSL_ACTION_SET && state == PRE_PARAMS_TO_CTRL)
        ...
        if (i == OSSL_NELEM(str_value_map)) {
            ...
            /* UCI_PKEY_CTRL_GET_RSA_PADDING weirdness explained further up */
...
/* UCI_PKEY_CTRL_RSA_PSS_SALTLEN, UCI_PKEY_CTRL_GET_RSA_PSS_SALTLEN */
                               ...
                               struct translation_ctx_st *ctx)
{
    ...
    if (state == PRE_CTRL_TO_PARAMS && ctx->action_type == OSSL_ACTION_GET) {
        /*
         * UCI_PKEY_CTRL_GET_RSA_PSS_SALTLEN returns the saltlen by filling
         * in the int pointed at by p2.  This is potentially as weird as
         * the way UCI_PKEY_CTRL_GET_RSA_PADDING works, except that saltlen
    ...
    if ((ctx->action_type == OSSL_ACTION_SET && state == PRE_PARAMS_TO_CTRL)
        ...
        if (state == POST_CTRL_TO_PARAMS) {
            /*
             * UCI_PKEY_CTRL_GET_RSA_PSS_SALTLEN weirdness explained further
             * up
             */
            *(int *)ctx->orig_p2 = val;
        } else {
...
/* UCI_PKEY_CTRL_HKDF_MODE */
static int fix_hkdf_mode(enum state state,
                         const struct translation_st *translation,
                         struct translation_ctx_st *ctx)
{
    static const OSSL_ITEM str_value_map[] = {
        { UCI_KDF_HKDF_MODE_EXTRACT_AND_EXPAND, "EXTRACT_AND_EXPAND" },
        { UCI_KDF_HKDF_MODE_EXTRACT_ONLY,       "EXTRACT_ONLY"       },
        { UCI_KDF_HKDF_MODE_EXPAND_ONLY,        "EXPAND_ONLY"        }
    };
    int ret;
                                  ...
                                  struct translation_ctx_st *ctx)
{
    ...
    case UCI_PKEY_DH:
        {
            const DH *dh = UCI_PKEY_get0_DH(pkey);
    ...
    case UCI_PKEY_EC:
        {
            const EC_GROUP *grp =
                EC_KEY_get0_group(UCI_PKEY_get0_EC_KEY(pkey));
            int nid = NID_undef;
                                  ...
                                  struct translation_ctx_st *ctx)
{
    ...
    case UCI_PKEY_DH:
        switch (ctx->params->data_type) {
        case OSSL_PARAM_OCTET_STRING:
            ctx->sz = ossl_dh_key2buf(UCI_PKEY_get0_DH(pkey), &buf, 0, 1);
            ctx->p2 = buf;
            break;
        case OSSL_PARAM_UNSIGNED_INTEGER:
            ctx->p2 = (void *)DH_get0_pub_key(UCI_PKEY_get0_DH(pkey));
            break;
        default:
            return 0;
        }
    ...
    case UCI_PKEY_DSA:
        if (ctx->params->data_type == OSSL_PARAM_UNSIGNED_INTEGER) {
            ctx->p2 = (void *)DSA_get0_pub_key(UCI_PKEY_get0_DSA(pkey));
            break;
        }
        return 0;
#endif
    ...
    case UCI_PKEY_EC:
        if (ctx->params->data_type == OSSL_PARAM_OCTET_STRING) {
            const EC_KEY *eckey = UCI_PKEY_get0_EC_KEY(pkey);
...
#ifndef OPENSSL_NO_DH
    case UCI_PKEY_DH:
        bn = DH_get0_p(UCI_PKEY_get0_DH(pkey));
        break;
#endif
#ifndef OPENSSL_NO_DSA
    case UCI_PKEY_DSA:
        bn = DSA_get0_p(UCI_PKEY_get0_DSA(pkey));
        break;
#endif
    default:
        ERR_raise(ERR_LIB_EVP, UCI_R_UNSUPPORTED_KEY_TYPE);
                                ...
                                struct translation_ctx_st *ctx)
{
    ...
    switch (UCI_PKEY_get_base_id(ctx->p2)) {
#ifndef OPENSSL_NO_DH
    case UCI_PKEY_DH:
        bn = DH_get0_q(UCI_PKEY_get0_DH(ctx->p2));
        break;
#endif
#ifndef OPENSSL_NO_DSA
    case UCI_PKEY_DSA:
        bn = DSA_get0_q(UCI_PKEY_get0_DSA(ctx->p2));
        break;
#endif
    }
...
#ifndef OPENSSL_NO_DH
    case UCI_PKEY_DH:
        bn = DH_get0_g(UCI_PKEY_get0_DH(ctx->p2));
        break;
#endif
#ifndef OPENSSL_NO_DSA
    case UCI_PKEY_DSA:
        bn = DSA_get0_g(UCI_PKEY_get0_DSA(ctx->p2));
        break;
#endif
    }
                                               ...
                                               struct translation_ctx_st *ctx)
{
    ...
    switch (UCI_PKEY_base_id(pkey)) {
#ifndef OPENSSL_NO_EC
    case UCI_PKEY_EC:
        val = EC_KEY_decoded_from_explicit_params(UCI_PKEY_get0_EC_KEY(pkey));
        if (val < 0) {
                             ...
                             struct translation_ctx_st *ctx)
{
    ...
    if (UCI_PKEY_get_base_id(ctx->p2) != UCI_PKEY_RSA
        && UCI_PKEY_get_base_id(ctx->p2) != UCI_PKEY_RSA_PSS)
        return 0;
    bn = RSA_get0_n(UCI_PKEY_get0_RSA(ctx->p2));
                             ...
                             struct translation_ctx_st *ctx)
{
    ...
    if (UCI_PKEY_get_base_id(ctx->p2) != UCI_PKEY_RSA
        && UCI_PKEY_get_base_id(ctx->p2) != UCI_PKEY_RSA_PSS)
        return 0;
    bn = RSA_get0_e(UCI_PKEY_get0_RSA(ctx->p2));
                             ...
                             struct translation_ctx_st *ctx)
{
    ...
    if (UCI_PKEY_get_base_id(ctx->p2) != UCI_PKEY_RSA
        && UCI_PKEY_get_base_id(ctx->p2) != UCI_PKEY_RSA_PSS)
        return 0;
    bn = RSA_get0_d(UCI_PKEY_get0_RSA(ctx->p2));
...
static int get_rsa_payload_factor(enum state state,
                                  ...
                                  size_t factornum)
{
    const RSA *r = UCI_PKEY_get0_RSA(ctx->p2);
    const BIGNUM *bn = NULL;
                         ...
                         struct translation_ctx_st *ctx)
{
    ...
    switch (state) {
    case PRE_PARAMS_TO_CTRL:
        if (!UCI_PKEY_CTX_IS_GEN_OP(ctx->pctx))
            return 0;
...
static const struct translation_st uci_pkey_ctx_translations[] = {
    /*
     * DistID: we pass it to the backend as an octet string,
     * but get it back as a pointer to an octet string.
     *
     * Note that the UCI_PKEY_CTRL_GET1_ID_LEN is purely for legacy purposes
     ...
     */
    { OSSL_ACTION_SET, -1, -1, UCI_PKEY_OP_TYPE_SIG,
      UCI_PKEY_CTRL_SET1_ID, "distid", "hexdistid",
      OSSL_PKEY_PARAM_DIST_ID, OSSL_PARAM_OCTET_STRING, NULL },
    { OSSL_ACTION_GET, -1, -1, -1,
      UCI_PKEY_CTRL_GET1_ID, "distid", "hexdistid",
      OSSL_PKEY_PARAM_DIST_ID, OSSL_PARAM_OCTET_PTR, NULL },
    { OSSL_ACTION_GET, -1, -1, -1,
      UCI_PKEY_CTRL_GET1_ID_LEN, NULL, NULL,
    ...
    /*
     * UCI_PKEY_CTRL_DH_KDF_TYPE is used both for setting and getting.  The
     * fixup function has to handle this...
     */
    { OSSL_ACTION_NONE, UCI_PKEY_DHX, 0, UCI_PKEY_OP_DERIVE,
      UCI_PKEY_CTRL_DH_KDF_TYPE, NULL, NULL,
      OSSL_EXCHANGE_PARAM_KDF_TYPE, OSSL_PARAM_UTF8_STRING,
      fix_dh_kdf_type },
    { OSSL_ACTION_SET, UCI_PKEY_DHX, 0, UCI_PKEY_OP_DERIVE,
      UCI_PKEY_CTRL_DH_KDF_MD, NULL, NULL,
      OSSL_EXCHANGE_PARAM_KDF_DIGEST, OSSL_PARAM_UTF8_STRING, fix_md },
    { OSSL_ACTION_GET, UCI_PKEY_DHX, 0, UCI_PKEY_OP_DERIVE,
      UCI_PKEY_CTRL_GET_DH_KDF_MD, NULL, NULL,
      OSSL_EXCHANGE_PARAM_KDF_DIGEST, OSSL_PARAM_UTF8_STRING, fix_md },
    { OSSL_ACTION_SET, UCI_PKEY_DHX, 0, UCI_PKEY_OP_DERIVE,
      UCI_PKEY_CTRL_DH_KDF_OUTLEN, NULL, NULL,
      OSSL_EXCHANGE_PARAM_KDF_OUTLEN, OSSL_PARAM_UNSIGNED_INTEGER, NULL },
    { OSSL_ACTION_GET, UCI_PKEY_DHX, 0, UCI_PKEY_OP_DERIVE,
      UCI_PKEY_CTRL_GET_DH_KDF_OUTLEN, NULL, NULL,
      OSSL_EXCHANGE_PARAM_KDF_OUTLEN, OSSL_PARAM_UNSIGNED_INTEGER, NULL },
    { OSSL_ACTION_SET, UCI_PKEY_DHX, 0, UCI_PKEY_OP_DERIVE,
      UCI_PKEY_CTRL_DH_KDF_UKM, NULL, NULL,
      OSSL_EXCHANGE_PARAM_KDF_UKM, OSSL_PARAM_OCTET_STRING, NULL },
    { OSSL_ACTION_GET, UCI_PKEY_DHX, 0, UCI_PKEY_OP_DERIVE,
      UCI_PKEY_CTRL_GET_DH_KDF_UKM, NULL, NULL,
      OSSL_EXCHANGE_PARAM_KDF_UKM, OSSL_PARAM_OCTET_PTR, NULL },
    { OSSL_ACTION_SET, UCI_PKEY_DHX, 0, UCI_PKEY_OP_DERIVE,
      UCI_PKEY_CTRL_DH_KDF_OID, NULL, NULL,
      OSSL_KDF_PARAM_CEK_ALG, OSSL_PARAM_UTF8_STRING, fix_oid },
    { OSSL_ACTION_GET, UCI_PKEY_DHX, 0, UCI_PKEY_OP_DERIVE,
      UCI_PKEY_CTRL_GET_DH_KDF_OID, NULL, NULL,
      OSSL_KDF_PARAM_CEK_ALG, OSSL_PARAM_UTF8_STRING, fix_oid },

    /* DHX Keygen Parameters that are shared with DH */
    { OSSL_ACTION_SET, UCI_PKEY_DHX, 0, UCI_PKEY_OP_PARAMGEN,
      UCI_PKEY_CTRL_DH_PARAMGEN_TYPE, "dh_paramgen_type", NULL,
      OSSL_PKEY_PARAM_FFC_TYPE, OSSL_PARAM_UTF8_STRING, fix_dh_paramgen_type },
    { OSSL_ACTION_SET, UCI_PKEY_DHX, 0, UCI_PKEY_OP_PARAMGEN,
      UCI_PKEY_CTRL_DH_PARAMGEN_PRIME_LEN, "dh_paramgen_prime_len", NULL,
      OSSL_PKEY_PARAM_FFC_PBITS, OSSL_PARAM_UNSIGNED_INTEGER, NULL },
    { OSSL_ACTION_SET, UCI_PKEY_DHX, 0, UCI_PKEY_OP_PARAMGEN | UCI_PKEY_OP_KEYGEN,
      UCI_PKEY_CTRL_DH_NID, "dh_param", NULL,
      OSSL_PKEY_PARAM_GROUP_NAME, OSSL_PARAM_UTF8_STRING, NULL },
    { OSSL_ACTION_SET, UCI_PKEY_DHX, 0, UCI_PKEY_OP_PARAMGEN | UCI_PKEY_OP_KEYGEN,
      UCI_PKEY_CTRL_DH_RFC5114, "dh_rfc5114", NULL,
      OSSL_PKEY_PARAM_GROUP_NAME, OSSL_PARAM_UTF8_STRING, fix_dh_nid5114 },

    /* DH Keygen Parameters that are shared with DHX */
    { OSSL_ACTION_SET, UCI_PKEY_DH, 0, UCI_PKEY_OP_PARAMGEN,
      UCI_PKEY_CTRL_DH_PARAMGEN_TYPE, "dh_paramgen_type", NULL,
      OSSL_PKEY_PARAM_FFC_TYPE, OSSL_PARAM_UTF8_STRING, fix_dh_paramgen_type },
    { OSSL_ACTION_SET, UCI_PKEY_DH, 0, UCI_PKEY_OP_PARAMGEN,
      UCI_PKEY_CTRL_DH_PARAMGEN_PRIME_LEN, "dh_paramgen_prime_len", NULL,
      OSSL_PKEY_PARAM_FFC_PBITS, OSSL_PARAM_UNSIGNED_INTEGER, NULL },
    { OSSL_ACTION_SET, UCI_PKEY_DH, 0, UCI_PKEY_OP_PARAMGEN | UCI_PKEY_OP_KEYGEN,
      UCI_PKEY_CTRL_DH_NID, "dh_param", NULL,
    ...
    { OSSL_ACTION_SET, UCI_PKEY_DH, 0, UCI_PKEY_OP_PARAMGEN,
      UCI_PKEY_CTRL_DH_PARAMGEN_GENERATOR, "dh_paramgen_generator", NULL,
      OSSL_PKEY_PARAM_DH_GENERATOR, OSSL_PARAM_INTEGER, NULL },

    /* DHX specific Keygen Parameters */
    { OSSL_ACTION_SET, UCI_PKEY_DHX, 0, UCI_PKEY_OP_PARAMGEN,
      UCI_PKEY_CTRL_DH_PARAMGEN_SUBPRIME_LEN, "dh_paramgen_subprime_len", NULL,
      OSSL_PKEY_PARAM_FFC_QBITS, OSSL_PARAM_UNSIGNED_INTEGER, NULL },

    { OSSL_ACTION_SET, UCI_PKEY_DH, 0, UCI_PKEY_OP_DERIVE,
      UCI_PKEY_CTRL_DH_PAD, "dh_pad", NULL,
     ...
     * ===
     */
    { OSSL_ACTION_SET, UCI_PKEY_DSA, 0, UCI_PKEY_OP_PARAMGEN,
      UCI_PKEY_CTRL_DSA_PARAMGEN_BITS, "dsa_paramgen_bits", NULL,
      OSSL_PKEY_PARAM_FFC_PBITS, OSSL_PARAM_UNSIGNED_INTEGER, NULL },
    { OSSL_ACTION_SET, UCI_PKEY_DSA, 0, UCI_PKEY_OP_PARAMGEN,
      UCI_PKEY_CTRL_DSA_PARAMGEN_Q_BITS, "dsa_paramgen_q_bits", NULL,
      OSSL_PKEY_PARAM_FFC_QBITS, OSSL_PARAM_UNSIGNED_INTEGER, NULL },
    { OSSL_ACTION_SET, UCI_PKEY_DSA, 0, UCI_PKEY_OP_PARAMGEN,
      UCI_PKEY_CTRL_DSA_PARAMGEN_MD, "dsa_paramgen_md", NULL,
     ...
     * ==
     */
    { OSSL_ACTION_SET, UCI_PKEY_EC, 0, UCI_PKEY_OP_PARAMGEN | UCI_PKEY_OP_KEYGEN,
      UCI_PKEY_CTRL_EC_PARAM_ENC, "ec_param_enc", NULL,
      OSSL_PKEY_PARAM_EC_ENCODING, OSSL_PARAM_UTF8_STRING, fix_ec_param_enc },
    { OSSL_ACTION_SET, UCI_PKEY_EC, 0, UCI_PKEY_OP_PARAMGEN | UCI_PKEY_OP_KEYGEN,
      UCI_PKEY_CTRL_EC_PARAMGEN_CURVE_NID, "ec_paramgen_curve", NULL,
      OSSL_PKEY_PARAM_GROUP_NAME, OSSL_PARAM_UTF8_STRING,
      fix_ec_paramgen_curve_nid },
    /*
     * UCI_PKEY_CTRL_EC_ECDH_COFACTOR and UCI_PKEY_CTRL_EC_KDF_TYPE are used
     * both for setting and getting.  The fixup function has to handle this...
     */
    { OSSL_ACTION_NONE, UCI_PKEY_EC, 0, UCI_PKEY_OP_DERIVE,
      UCI_PKEY_CTRL_EC_ECDH_COFACTOR, "ecdh_cofactor_mode", NULL,
      OSSL_EXCHANGE_PARAM_EC_ECDH_COFACTOR_MODE, OSSL_PARAM_INTEGER,
      fix_ecdh_cofactor },
    { OSSL_ACTION_NONE, UCI_PKEY_EC, 0, UCI_PKEY_OP_DERIVE,
      UCI_PKEY_CTRL_EC_KDF_TYPE, NULL, NULL,
      OSSL_EXCHANGE_PARAM_KDF_TYPE, OSSL_PARAM_UTF8_STRING, fix_ec_kdf_type },
    { OSSL_ACTION_SET, UCI_PKEY_EC, 0, UCI_PKEY_OP_DERIVE,
      UCI_PKEY_CTRL_EC_KDF_MD, "ecdh_kdf_md", NULL,
      OSSL_EXCHANGE_PARAM_KDF_DIGEST, OSSL_PARAM_UTF8_STRING, fix_md },
    { OSSL_ACTION_GET, UCI_PKEY_EC, 0, UCI_PKEY_OP_DERIVE,
      UCI_PKEY_CTRL_GET_EC_KDF_MD, NULL, NULL,
      OSSL_EXCHANGE_PARAM_KDF_DIGEST, OSSL_PARAM_UTF8_STRING, fix_md },
    { OSSL_ACTION_SET, UCI_PKEY_EC, 0, UCI_PKEY_OP_DERIVE,
      UCI_PKEY_CTRL_EC_KDF_OUTLEN, NULL, NULL,
      OSSL_EXCHANGE_PARAM_KDF_OUTLEN, OSSL_PARAM_UNSIGNED_INTEGER, NULL },
    { OSSL_ACTION_GET, UCI_PKEY_EC, 0, UCI_PKEY_OP_DERIVE,
      UCI_PKEY_CTRL_GET_EC_KDF_OUTLEN, NULL, NULL,
      OSSL_EXCHANGE_PARAM_KDF_OUTLEN, OSSL_PARAM_UNSIGNED_INTEGER, NULL },
    { OSSL_ACTION_SET, UCI_PKEY_EC, 0, UCI_PKEY_OP_DERIVE,
      UCI_PKEY_CTRL_EC_KDF_UKM, NULL, NULL,
      OSSL_EXCHANGE_PARAM_KDF_UKM, OSSL_PARAM_OCTET_STRING, NULL },
    { OSSL_ACTION_GET, UCI_PKEY_EC, 0, UCI_PKEY_OP_DERIVE,
      UCI_PKEY_CTRL_GET_EC_KDF_UKM, NULL, NULL,
      OSSL_EXCHANGE_PARAM_KDF_UKM, OSSL_PARAM_OCTET_PTR, NULL },
     ...
     * ==
     */
    { OSSL_ACTION_SET, UCI_PKEY_SM2, 0, UCI_PKEY_OP_PARAMGEN | UCI_PKEY_OP_KEYGEN,
      UCI_PKEY_CTRL_EC_PARAM_ENC, "ec_param_enc", NULL,
      OSSL_PKEY_PARAM_EC_ENCODING, OSSL_PARAM_UTF8_STRING, fix_ec_param_enc },
    { OSSL_ACTION_SET, UCI_PKEY_SM2, 0, UCI_PKEY_OP_PARAMGEN | UCI_PKEY_OP_KEYGEN,
      UCI_PKEY_CTRL_EC_PARAMGEN_CURVE_NID, "ec_paramgen_curve", NULL,
     ...
     * both for setting and getting.  The fixup function has to handle this...
     */
    { OSSL_ACTION_NONE, UCI_PKEY_SM2, 0, UCI_PKEY_OP_DERIVE,
      UCI_PKEY_CTRL_EC_ECDH_COFACTOR, "ecdh_cofactor_mode", NULL,
      OSSL_EXCHANGE_PARAM_EC_ECDH_COFACTOR_MODE, OSSL_PARAM_INTEGER,
      fix_ecdh_cofactor },
    { OSSL_ACTION_NONE, UCI_PKEY_SM2, 0, UCI_PKEY_OP_DERIVE,
      UCI_PKEY_CTRL_EC_KDF_TYPE, NULL, NULL,
      OSSL_EXCHANGE_PARAM_KDF_TYPE, OSSL_PARAM_UTF8_STRING, fix_ec_kdf_type },
    { OSSL_ACTION_SET, UCI_PKEY_SM2, 0, UCI_PKEY_OP_DERIVE,
      UCI_PKEY_CTRL_EC_KDF_MD, "ecdh_kdf_md", NULL,
      OSSL_EXCHANGE_PARAM_KDF_DIGEST, OSSL_PARAM_UTF8_STRING, fix_md },
    { OSSL_ACTION_GET, UCI_PKEY_SM2, 0, UCI_PKEY_OP_DERIVE,
      UCI_PKEY_CTRL_GET_EC_KDF_MD, NULL, NULL,
      OSSL_EXCHANGE_PARAM_KDF_DIGEST, OSSL_PARAM_UTF8_STRING, fix_md },
    { OSSL_ACTION_SET, UCI_PKEY_SM2, 0, UCI_PKEY_OP_DERIVE,
      UCI_PKEY_CTRL_EC_KDF_OUTLEN, NULL, NULL,
     ...
     * idea.
     */
    { OSSL_ACTION_SET, UCI_PKEY_RSA, UCI_PKEY_RSA_PSS,
      UCI_PKEY_OP_TYPE_CRYPT | UCI_PKEY_OP_TYPE_SIG,
      UCI_PKEY_CTRL_RSA_PADDING, "rsa_padding_mode", NULL,
      OSSL_PKEY_PARAM_PAD_MODE, OSSL_PARAM_UTF8_STRING, fix_rsa_padding_mode },
    { OSSL_ACTION_GET, UCI_PKEY_RSA, UCI_PKEY_RSA_PSS,
      UCI_PKEY_OP_TYPE_CRYPT | UCI_PKEY_OP_TYPE_SIG,
      UCI_PKEY_CTRL_GET_RSA_PADDING, NULL, NULL,
      OSSL_PKEY_PARAM_PAD_MODE, OSSL_PARAM_UTF8_STRING, fix_rsa_padding_mode },

    { OSSL_ACTION_SET, UCI_PKEY_RSA, UCI_PKEY_RSA_PSS,
      UCI_PKEY_OP_TYPE_CRYPT | UCI_PKEY_OP_TYPE_SIG,
      UCI_PKEY_CTRL_RSA_MGF1_MD, "rsa_mgf1_md", NULL,
      OSSL_PKEY_PARAM_MGF1_DIGEST, OSSL_PARAM_UTF8_STRING, fix_md },
    { OSSL_ACTION_GET, UCI_PKEY_RSA, UCI_PKEY_RSA_PSS,
      UCI_PKEY_OP_TYPE_CRYPT | UCI_PKEY_OP_TYPE_SIG,
      UCI_PKEY_CTRL_GET_RSA_MGF1_MD, NULL, NULL,
      OSSL_PKEY_PARAM_MGF1_DIGEST, OSSL_PARAM_UTF8_STRING, fix_md },
     ...
     */
    { OSSL_ACTION_SET, UCI_PKEY_RSA, UCI_PKEY_RSA_PSS, UCI_PKEY_OP_TYPE_SIG,
      UCI_PKEY_CTRL_RSA_PSS_SALTLEN, "rsa_pss_saltlen", NULL,
      OSSL_PKEY_PARAM_RSA_PSS_SALTLEN, OSSL_PARAM_UTF8_STRING,
      fix_rsa_pss_saltlen },
    { OSSL_ACTION_GET, UCI_PKEY_RSA, UCI_PKEY_RSA_PSS, UCI_PKEY_OP_TYPE_SIG,
      UCI_PKEY_CTRL_GET_RSA_PSS_SALTLEN, NULL, NULL,
      OSSL_PKEY_PARAM_RSA_PSS_SALTLEN, OSSL_PARAM_UTF8_STRING,
      fix_rsa_pss_saltlen },

    { OSSL_ACTION_SET, UCI_PKEY_RSA, 0, UCI_PKEY_OP_TYPE_CRYPT,
      UCI_PKEY_CTRL_RSA_OAEP_MD, "rsa_oaep_md", NULL,
      OSSL_ASYM_CIPHER_PARAM_OAEP_DIGEST, OSSL_PARAM_UTF8_STRING, fix_md },
    { OSSL_ACTION_GET, UCI_PKEY_RSA, 0, UCI_PKEY_OP_TYPE_CRYPT,
      UCI_PKEY_CTRL_GET_RSA_OAEP_MD, NULL, NULL,
      OSSL_ASYM_CIPHER_PARAM_OAEP_DIGEST, OSSL_PARAM_UTF8_STRING, fix_md },
    /*
     ...
     */
    { OSSL_ACTION_SET, UCI_PKEY_RSA, 0, UCI_PKEY_OP_TYPE_CRYPT,
      UCI_PKEY_CTRL_RSA_OAEP_LABEL, NULL, "rsa_oaep_label",
      OSSL_ASYM_CIPHER_PARAM_OAEP_LABEL, OSSL_PARAM_OCTET_STRING, NULL },
    { OSSL_ACTION_GET, UCI_PKEY_RSA, 0, UCI_PKEY_OP_TYPE_CRYPT,
      UCI_PKEY_CTRL_GET_RSA_OAEP_LABEL, NULL, NULL,
      OSSL_ASYM_CIPHER_PARAM_OAEP_LABEL, OSSL_PARAM_OCTET_PTR, NULL },

    { OSSL_ACTION_SET, UCI_PKEY_RSA, 0, UCI_PKEY_OP_TYPE_CRYPT,
      UCI_PKEY_CTRL_RSA_IMPLICIT_REJECTION, NULL,
      "rsa_pkcs1_implicit_rejection",
      OSSL_ASYM_CIPHER_PARAM_IMPLICIT_REJECTION, OSSL_PARAM_UNSIGNED_INTEGER,
      NULL },

    { OSSL_ACTION_SET, UCI_PKEY_RSA_PSS, 0, UCI_PKEY_OP_TYPE_GEN,
      UCI_PKEY_CTRL_MD, "rsa_pss_keygen_md", NULL,
      OSSL_ALG_PARAM_DIGEST, OSSL_PARAM_UTF8_STRING, fix_md },
    { OSSL_ACTION_SET, UCI_PKEY_RSA_PSS, 0, UCI_PKEY_OP_TYPE_GEN,
      UCI_PKEY_CTRL_RSA_MGF1_MD, "rsa_pss_keygen_mgf1_md", NULL,
      OSSL_PKEY_PARAM_MGF1_DIGEST, OSSL_PARAM_UTF8_STRING, fix_md },
    { OSSL_ACTION_SET, UCI_PKEY_RSA_PSS, 0, UCI_PKEY_OP_TYPE_GEN,
      UCI_PKEY_CTRL_RSA_PSS_SALTLEN, "rsa_pss_keygen_saltlen", NULL,
      OSSL_SIGNATURE_PARAM_PSS_SALTLEN, OSSL_PARAM_INTEGER, NULL },
    { OSSL_ACTION_SET, UCI_PKEY_RSA, UCI_PKEY_RSA_PSS, UCI_PKEY_OP_KEYGEN,
      UCI_PKEY_CTRL_RSA_KEYGEN_BITS, "rsa_keygen_bits", NULL,
      OSSL_PKEY_PARAM_RSA_BITS, OSSL_PARAM_UNSIGNED_INTEGER, NULL },
    { OSSL_ACTION_SET, UCI_PKEY_RSA, UCI_PKEY_RSA_PSS, UCI_PKEY_OP_KEYGEN,
      UCI_PKEY_CTRL_RSA_KEYGEN_PUBEXP, "rsa_keygen_pubexp", NULL,
      OSSL_PKEY_PARAM_RSA_E, OSSL_PARAM_UNSIGNED_INTEGER, NULL },
    { OSSL_ACTION_SET, UCI_PKEY_RSA, UCI_PKEY_RSA_PSS, UCI_PKEY_OP_KEYGEN,
      UCI_PKEY_CTRL_RSA_KEYGEN_PRIMES, "rsa_keygen_primes", NULL,
     ...
     */
    { OSSL_ACTION_SET, -1, -1, UCI_PKEY_OP_TYPE_SIG,
      UCI_PKEY_CTRL_SET_DIGEST_SIZE, "digestsize", NULL,
     ...
     * ========
     */
    { OSSL_ACTION_SET, -1, -1, UCI_PKEY_OP_DERIVE,
      UCI_PKEY_CTRL_TLS_MD, "md", NULL,
      OSSL_KDF_PARAM_DIGEST, OSSL_PARAM_UTF8_STRING, fix_md },
    { OSSL_ACTION_SET, -1, -1, UCI_PKEY_OP_DERIVE,
      UCI_PKEY_CTRL_TLS_SECRET, "secret", "hexsecret",
      OSSL_KDF_PARAM_SECRET, OSSL_PARAM_OCTET_STRING, NULL },
    { OSSL_ACTION_SET, -1, -1, UCI_PKEY_OP_DERIVE,
      UCI_PKEY_CTRL_TLS_SEED, "seed", "hexseed",
     ...
     */
    { OSSL_ACTION_SET, -1, -1, UCI_PKEY_OP_DERIVE,
      UCI_PKEY_CTRL_HKDF_MD, "md", NULL,
      OSSL_KDF_PARAM_DIGEST, OSSL_PARAM_UTF8_STRING, fix_md },
    { OSSL_ACTION_SET, -1, -1, UCI_PKEY_OP_DERIVE,
      UCI_PKEY_CTRL_HKDF_SALT, "salt", "hexsalt",
      OSSL_KDF_PARAM_SALT, OSSL_PARAM_OCTET_STRING, NULL },
    { OSSL_ACTION_SET, -1, -1, UCI_PKEY_OP_DERIVE,
      UCI_PKEY_CTRL_HKDF_KEY, "key", "hexkey",
      OSSL_KDF_PARAM_KEY, OSSL_PARAM_OCTET_STRING, NULL },
    { OSSL_ACTION_SET, -1, -1, UCI_PKEY_OP_DERIVE,
      UCI_PKEY_CTRL_HKDF_INFO, "info", "hexinfo",
     ...
     */
    { OSSL_ACTION_SET, -1, -1, UCI_PKEY_OP_DERIVE,
      UCI_PKEY_CTRL_PASS, "pass", "hexpass",
      OSSL_KDF_PARAM_PASSWORD, OSSL_PARAM_OCTET_STRING, NULL },
    { OSSL_ACTION_SET, -1, -1, UCI_PKEY_OP_DERIVE,
      UCI_PKEY_CTRL_SCRYPT_SALT, "salt", "hexsalt",
      OSSL_KDF_PARAM_SALT, OSSL_PARAM_OCTET_STRING, NULL },
    { OSSL_ACTION_SET, -1, -1, UCI_PKEY_OP_DERIVE,
      UCI_PKEY_CTRL_SCRYPT_N, "N", NULL,
      OSSL_KDF_PARAM_SCRYPT_N, OSSL_PARAM_UNSIGNED_INTEGER, NULL },
    { OSSL_ACTION_SET, -1, -1, UCI_PKEY_OP_DERIVE,
      UCI_PKEY_CTRL_SCRYPT_R, "r", NULL,
      OSSL_KDF_PARAM_SCRYPT_R, OSSL_PARAM_UNSIGNED_INTEGER, NULL },
    { OSSL_ACTION_SET, -1, -1, UCI_PKEY_OP_DERIVE,
      UCI_PKEY_CTRL_SCRYPT_P, "p", NULL,
      OSSL_KDF_PARAM_SCRYPT_P, OSSL_PARAM_UNSIGNED_INTEGER, NULL },
    { OSSL_ACTION_SET, -1, -1, UCI_PKEY_OP_DERIVE,
      UCI_PKEY_CTRL_SCRYPT_MAXMEM_BYTES, "maxmem_bytes", NULL,
      OSSL_KDF_PARAM_SCRYPT_MAXMEM, OSSL_PARAM_UNSIGNED_INTEGER, NULL },

    { OSSL_ACTION_SET, -1, -1, UCI_PKEY_OP_KEYGEN | UCI_PKEY_OP_TYPE_CRYPT,
      UCI_PKEY_CTRL_CIPHER, NULL, NULL,
      OSSL_PKEY_PARAM_CIPHER, OSSL_PARAM_UTF8_STRING, fix_cipher },
    { OSSL_ACTION_SET, -1, -1, UCI_PKEY_OP_KEYGEN,
      UCI_PKEY_CTRL_SET_MAC_KEY, "key", "hexkey",
      OSSL_PKEY_PARAM_PRIV_KEY, OSSL_PARAM_OCTET_STRING, NULL },

    { OSSL_ACTION_SET, -1, -1, UCI_PKEY_OP_TYPE_SIG,
      UCI_PKEY_CTRL_MD, NULL, NULL,
      OSSL_SIGNATURE_PARAM_DIGEST, OSSL_PARAM_UTF8_STRING, fix_md },
    { OSSL_ACTION_GET, -1, -1, UCI_PKEY_OP_TYPE_SIG,
      UCI_PKEY_CTRL_GET_MD, NULL, NULL,
      OSSL_SIGNATURE_PARAM_DIGEST, OSSL_PARAM_UTF8_STRING, fix_md },
     ...
     * ===
     */
    { OSSL_ACTION_SET, UCI_PKEY_X25519, UCI_PKEY_X25519, UCI_PKEY_OP_KEYGEN, -1, NULL, NULL,
      OSSL_PKEY_PARAM_GROUP_NAME, OSSL_PARAM_UTF8_STRING, fix_group_ecx },
    { OSSL_ACTION_SET, UCI_PKEY_X25519, UCI_PKEY_X25519, UCI_PKEY_OP_PARAMGEN, -1, NULL, NULL,
      OSSL_PKEY_PARAM_GROUP_NAME, OSSL_PARAM_UTF8_STRING, fix_group_ecx },
    { OSSL_ACTION_SET, UCI_PKEY_X448, UCI_PKEY_X448, UCI_PKEY_OP_KEYGEN, -1, NULL, NULL,
      OSSL_PKEY_PARAM_GROUP_NAME, OSSL_PARAM_UTF8_STRING, fix_group_ecx },
    { OSSL_ACTION_SET, UCI_PKEY_X448, UCI_PKEY_X448, UCI_PKEY_OP_PARAMGEN, -1, NULL, NULL,
      OSSL_PKEY_PARAM_GROUP_NAME, OSSL_PARAM_UTF8_STRING, fix_group_ecx },
};

static const struct translation_st uci_pkey_translations[] = {
    /*
     ...
     * expect to get the UCI_PKEY pointer through |ctx->p2|.
     */
...
static const struct translation_st *
lookup_uci_pkey_ctx_translation(struct translation_st *tmpl)
{
    return lookup_translation(tmpl, uci_pkey_ctx_translations,
                              OSSL_NELEM(uci_pkey_ctx_translations));
}

static const struct translation_st *
lookup_uci_pkey_translation(struct translation_st *tmpl)
{
    return lookup_translation(tmpl, uci_pkey_translations,
                              OSSL_NELEM(uci_pkey_translations));
}

/* This must ONLY be called for provider side operations */
int uci_pkey_ctx_ctrl_to_param(UCI_PKEY_CTX *pctx,
                               ...
                               int cmd, int p1, void *p2)
{
    ...
    tmpl.optype = optype;
    translation = lookup_uci_pkey_ctx_translation(&tmpl);
    ...
    if (ret > 0) {
        ...
        case OSSL_ACTION_GET:
            ret = uci_pkey_ctx_get_params_strict(pctx, ctx.params);
            break;
        case OSSL_ACTION_SET:
            ret = uci_pkey_ctx_set_params_strict(pctx, ctx.params);
            break;
        }
    }
...
/* This must ONLY be called for provider side operations */
int uci_pkey_ctx_ctrl_str_to_param(UCI_PKEY_CTX *pctx,
...
static int uci_pkey_ctx_setget_params_to_ctrl(UCI_PKEY_CTX *pctx,
                                              enum action action_type,
                                              OSSL_PARAM *params)
{
    int keytype = pctx->legacy_keytype;
    ...
    for (; params != NULL && params->key != NULL; params++) {
        ...
        if (ret > 0 && ctx.action_type != OSSL_ACTION_NONE)
            ret = UCI_PKEY_CTX_ctrl(pctx, keytype, optype,
         ...
         *
         * NOTE: even though UCI_PKEY_CTX_ctrl return value is documented
...
int uci_pkey_ctx_set_params_to_ctrl(UCI_PKEY_CTX *ctx, const OSSL_PARAM *params)
{
    if (ctx->keymgmt != NULL)
        return 0;
    return uci_pkey_ctx_setget_params_to_ctrl(ctx, OSSL_ACTION_SET, (OSSL_PARAM *)params);
}

int uci_pkey_ctx_get_params_to_ctrl(UCI_PKEY_CTX *ctx, OSSL_PARAM *params)
{
    if (ctx->keymgmt != NULL)
        return 0;
    return uci_pkey_ctx_setget_params_to_ctrl(ctx, OSSL_ACTION_GET, params);
}

/* This must ONLY be called for legacy UCI_PKEYs */
static int uci_pkey_setget_params_to_ctrl(const UCI_PKEY *pkey,
                                          enum action action_type,
                                          OSSL_PARAM *params)
{
    int ret = 1;
    ...
    for (; params != NULL && params->key != NULL; params++) {
        ...
        tmpl.param_key = params->key;
        translation = lookup_uci_pkey_translation(&tmpl);
        ...
        /*
         * UCI_PKEY doesn't have any ctrl function, so we rely completely
...
int uci_pkey_get_params_to_ctrl(const UCI_PKEY *pkey, OSSL_PARAM *params)
{
    return uci_pkey_setget_params_to_ctrl(pkey, OSSL_ACTION_GET, params);
}
