...
#define BUF_OFFSET      (ENC_MIN_CHUNK + UCI_MAX_BLOCK_LENGTH)
...
typedef struct enc_struct {
    ...
    int finished;
    int ok;                     /* bad decrypt */
    UCI_CIPHER_CTX *cipher;
    unsigned char *read_start, *read_end;
    /*
     * buf is larger than ENC_BLOCK_SIZE because UCI_DecryptUpdate can return
...
static int enc_read(BIO *b, char *out, int outl)
{
    ...
    blocksize = UCI_CIPHER_CTX_get_block_size(ctx->cipher);
    ...
    while (outl > 0) {
        ...
        if (i <= 0) {
            ...
            if (!BIO_should_retry(next)) {
                ctx->cont = i;
                ctx->finished = 1;
                i = UCI_CipherFinal_ex(ctx->cipher,
                                       ctx->buf, &(ctx->buf_len));
                ctx->ok = i;
                ctx->buf_off = 0;
            } else {
            ...
            if (outl > ENC_MIN_CHUNK) {
                ...
                if (!UCI_CipherUpdate(ctx->cipher,
            ...
            ctx->cont = 1;
            /*
             * Note: it is possible for UCI_CipherUpdate to decrypt zero
...
static long enc_ctrl(BIO *b, int cmd, long num, void *ptr)
{
    ...
    case BIO_CTRL_RESET:
        ctx->ok = 1;
        ctx->finished = 0;
        if (!UCI_CipherInit_ex(ctx->cipher, NULL, NULL, NULL, NULL,
                               UCI_CIPHER_CTX_is_encrypting(ctx->cipher)))
            return 0;
 ...
 again:
        ...
        if (!ctx->finished) {
            ctx->finished = 1;
            ctx->buf_off = 0;
            ret = UCI_CipherFinal_ex(ctx->cipher,
    ...
    case BIO_CTRL_DUP:
        ...
        dctx->cipher = UCI_CIPHER_CTX_new();
        if (dctx->cipher == NULL)
            return 0;
        ret = UCI_CIPHER_CTX_copy(dctx->cipher, ctx->cipher);
        if (ret)
            BIO_set_init(dbio, 1);
        break;
    default:
...
int BIO_set_cipher(BIO *b, const UCI_CIPHER *c, const unsigned char *k,
                   const unsigned char *i, int e)
{
    ...
    if (!UCI_CipherInit_ex(ctx->cipher, c, NULL, k, i, e))
        return 0;
