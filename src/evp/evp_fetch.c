...
#include "crypto/evp.h"    /* uci_local.h needs it */
#include "uci_local.h"

#define NAME_SEPARATOR ':'

/* Data to be passed through ossl_method_construct() */
struct uci_method_data_st {
    OSSL_LIB_CTX *libctx;
    int operation_id;            /* For get_uci_method_from_store() */
    int name_id;                 /* For get_uci_method_from_store() */
    const char *names;           /* For get_uci_method_from_store() */
    const char *propquery;       /* For get_uci_method_from_store() */

    OSSL_METHOD_STORE *tmp_store; /* For get_tmp_uci_method_store() */
 ...
 */
static void *get_tmp_uci_method_store(void *data)
{
    struct uci_method_data_st *methdata = data;
 ...
 static void dealloc_tmp_uci_method_store(void *store)
{
...
static OSSL_METHOD_STORE *get_uci_method_store(OSSL_LIB_CTX *libctx)
{
    return ossl_lib_ctx_get_data(libctx, OSSL_LIB_CTX_UCI_METHOD_STORE_INDEX);
}

static int reserve_uci_method_store(void *store, void *data)
{
    struct uci_method_data_st *methdata = data;

    if (store == NULL
        && (store = get_uci_method_store(methdata->libctx)) == NULL)
        return 0;

    return ossl_method_lock_store(store);
}

static int unreserve_uci_method_store(void *store, void *data)
{
    struct uci_method_data_st *methdata = data;
...
static uint32_t uci_method_id(int name_id, unsigned int operation_id)
{
...
static void *get_uci_method_from_store(void *store, const OSSL_PROVIDER **prov,
                                       void *data)
{
    struct uci_method_data_st *methdata = data;
    void *method = NULL;
    ...
    /*
     * get_uci_method_from_store() is only called to try and get the method
     * that uci_generic_fetch() is asking for, and the operation id as well
    ...
    if (name_id == 0
        || (meth_id = uci_method_id(name_id, methdata->operation_id)) == 0)
        return NULL;
...
static int put_uci_method_in_store(void *store, void *method,
                                   ...
                                   void *data)
{
    ...
    /*
     * put_uci_method_in_store() is only called with an EVP method that was
    ...
    OSSL_TRACE1(QUERY, "put_uci_method_in_store: original store: %p\n", store);
    if (store == NULL
        && (store = get_uci_method_store(methdata->libctx)) == NULL)
        return 0;

    OSSL_TRACE5(QUERY,
                "put_uci_method_in_store: "
 ...
 */
static void *construct_uci_method(const OSSL_ALGORITHM *algodef,
                                  OSSL_PROVIDER *prov, void *data)
{
    /*
     * This function is only called if get_uci_method_from_store() returned
     ...
     * helps inner_uci_generic_fetch() determine what error it should
...
static void destruct_uci_method(void *method, void *data)
{
    struct uci_method_data_st *methdata = data;
...
static void *
inner_uci_generic_fetch(struct uci_method_data_st *methdata,
...
{
    OSSL_METHOD_STORE *store = get_uci_method_store(methdata->libctx);
    OSSL_NAMEMAP *namemap = ossl_namemap_stored(methdata->libctx);
#ifdef FIPS_MODULE
    /*
    ...
    /*
     * If we have a name id, calculate a method id with uci_method_id().
     *
     * uci_method_id returns 0 if we have too many operations (more than
     ...
     */
    if (name_id != 0 && (meth_id = uci_method_id(name_id, operation_id)) == 0) {
        ERR_raise(ERR_LIB_EVP, ERR_R_INTERNAL_ERROR);
        return NULL;
    }
    ...
    if (meth_id == 0
        ...
        OSSL_METHOD_CONSTRUCT_METHOD mcm = {
            get_tmp_uci_method_store,
            reserve_uci_method_store,
            unreserve_uci_method_store,
            get_uci_method_from_store,
            put_uci_method_in_store,
            construct_uci_method,
            destruct_uci_method
        };
        ...
        if ((method = ossl_method_construct(methdata->libctx, operation_id,
             ...
             * already been calculated in get_uci_method_from_store() and
             * put_uci_method_in_store() above.
             ...
             * rather than introducing new features where in the UCI_<obj>_fetch
            ...
            if (name_id == 0) {
                ...
                meth_id = uci_method_id(name_id, operation_id);
                if (meth_id != 0)
...
void *uci_generic_fetch(OSSL_LIB_CTX *libctx, int operation_id,
                        ...
                        void (*free_method)(void *))
{
    struct uci_method_data_st methdata;
    void *method;

    methdata.libctx = libctx;
    methdata.tmp_store = NULL;
    method = inner_uci_generic_fetch(&methdata, NULL, operation_id,
                                     name, properties,
                                     new_method, up_ref_method, free_method);
    dealloc_tmp_uci_method_store(methdata.tmp_store);
    return method;
}

/*
 * uci_generic_fetch_from_prov() is special, and only returns methods from
 * the given provider.
 * This is meant to be used when one method needs to fetch an associated
 * method.
 */
void *uci_generic_fetch_from_prov(OSSL_PROVIDER *prov, int operation_id,
                                  const char *name, const char *properties,
                                  void *(*new_method)(int name_id,
                                                      const OSSL_ALGORITHM *algodef,
                                                      OSSL_PROVIDER *prov),
                                  ...
                                  void (*free_method)(void *))
{
    ...
    methdata.tmp_store = NULL;
    method = inner_uci_generic_fetch(&methdata, prov, operation_id,
                                     name, properties,
                                     new_method, up_ref_method, free_method);
    dealloc_tmp_uci_method_store(methdata.tmp_store);
    return method;
...
int uci_method_store_cache_flush(OSSL_LIB_CTX *libctx)
{
    OSSL_METHOD_STORE *store = get_uci_method_store(libctx);
...
int uci_method_store_remove_all_provided(const OSSL_PROVIDER *prov)
{
    OSSL_LIB_CTX *libctx = ossl_provider_libctx(prov);
    OSSL_METHOD_STORE *store = get_uci_method_store(libctx);
...
static int uci_set_parsed_default_properties(OSSL_LIB_CTX *libctx,
...
int uci_set_default_properties_int(OSSL_LIB_CTX *libctx, const char *propq,
                                   int loadconfig, int mirrored)
{
    OSSL_PROPERTY_LIST *pl = NULL;

    if (propq != NULL && (pl = ossl_parse_query(libctx, propq, 1)) == NULL) {
        ERR_raise(ERR_LIB_EVP, UCI_R_DEFAULT_QUERY_PARSE_ERROR);
        return 0;
    }
    if (!uci_set_parsed_default_properties(libctx, pl, loadconfig, mirrored)) {
...
{
    return uci_set_default_properties_int(libctx, propq, 1, 0);
}

static int uci_default_properties_merge(OSSL_LIB_CTX *libctx, const char *propq,
                                        int loadconfig)
{
    OSSL_PROPERTY_LIST **plp = ossl_ctx_global_properties(libctx, loadconfig);
    ...
    if (plp == NULL || *plp == NULL)
        return uci_set_default_properties_int(libctx, propq, 0, 0);
    ...
    if (!uci_set_parsed_default_properties(libctx, pl2, 0, 0)) {
...
static int uci_default_property_is_enabled(OSSL_LIB_CTX *libctx,
...
{
    return uci_default_property_is_enabled(libctx, "fips");
}

int uci_default_properties_enable_fips_int(OSSL_LIB_CTX *libctx, int enable,
                                           int loadconfig)
{
    const char *query = (enable != 0) ? "fips=yes" : "-fips";

    return uci_default_properties_merge(libctx, query, loadconfig);
}

int UCI_default_properties_enable_fips(OSSL_LIB_CTX *libctx, int enable)
{
    return uci_default_properties_enable_fips_int(libctx, enable, 1);
}

char *uci_get_global_properties_str(OSSL_LIB_CTX *libctx, int loadconfig)
{
    OSSL_PROPERTY_LIST **plp = ossl_ctx_global_properties(libctx, loadconfig);
    char *propstr = NULL;
    size_t sz;
...
char *UCI_get1_default_properties(OSSL_LIB_CTX *libctx)
{
    return uci_get_global_properties_str(libctx, ossl_lib_ctx_is_global_default(libctx));
}
...
void uci_generic_do_all(OSSL_LIB_CTX *libctx, int operation_id,
                        void (*user_fn)(void *method, void *arg),
                        void *user_arg,
                        void *(*new_method)(int name_id,
                                            const OSSL_ALGORITHM *algodef,
                        ...
                        void (*free_method)(void *))
{
    ...
    methdata.libctx = libctx;
    methdata.tmp_store = NULL;
    (void)inner_uci_generic_fetch(&methdata, NULL, operation_id, NULL, NULL,
                                  new_method, up_ref_method, free_method);
    ...
    if (methdata.tmp_store != NULL)
        ossl_method_store_do_all(methdata.tmp_store, &filter_on_operation_id,
                                 &data);
    ossl_method_store_do_all(get_uci_method_store(libctx),
                             &filter_on_operation_id, &data);
    dealloc_tmp_uci_method_store(methdata.tmp_store);
}

int uci_is_a(OSSL_PROVIDER *prov, int number,
...
int uci_names_do_all(OSSL_PROVIDER *prov, int number,
