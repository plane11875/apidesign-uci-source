...
int UCI_PBE_scrypt_ex(const char *pass, size_t passlen,
                      ...
                      OSSL_LIB_CTX *ctx, const char *propq)
{
    ...
    if (r > UINT32_MAX || p > UINT32_MAX) {
        ERR_raise(ERR_LIB_EVP, UCI_R_PARAMETER_TOO_LARGE);
        return 0;
    }
    ...
    kdf = UCI_KDF_fetch(ctx, OSSL_KDF_NAME_SCRYPT, propq);
    ...
    *z = OSSL_PARAM_construct_end();
    if (UCI_KDF_derive(kctx, key, keylen, params) != 1)
        rv = 0;
...
int UCI_PBE_scrypt(const char *pass, size_t passlen,
                   const unsigned char *salt, size_t saltlen,
                   uint64_t N, uint64_t r, uint64_t p, uint64_t maxmem,
                   unsigned char *key, size_t keylen)
{
    return UCI_PBE_scrypt_ex(pass, passlen, salt, saltlen, N, r, p, maxmem,
