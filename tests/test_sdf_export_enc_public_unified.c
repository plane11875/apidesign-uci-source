...
static int test_rsa(HANDLE sess)
{
    ...
    rc_old = SDF_ExportEncPublicKey_RSA(sess, 1, &old_pub);
    rc_new = SDF_ExportEncPublicKey(sess, SGD_RSA, 1, (BYTE *)&new_pub, &new_len);
    ...
    print_sha256_hex("ExportEnc.RSA", (const BYTE *)&new_pub, (ULONG)sizeof(new_pub));
    printf("[PASS] ExportEnc RSA unified matches legacy\n");
    return 1;
}
...
static int test_ecc(HANDLE sess)
{
    ...
    rc_old = SDF_ExportEncPublicKey_ECC(sess, 1, &old_pub);
    rc_new = SDF_ExportEncPublicKey(sess, SGD_SM2_3, 1, (BYTE *)&new_pub, &new_len);
    ...
    print_sha256_hex("ExportEnc.ECC", (const BYTE *)&new_pub, (ULONG)sizeof(new_pub));
    printf("[PASS] ExportEnc ECC unified matches legacy\n");
    return 1;
}
...
int main(void)
{
    ...
    if (ok_rsa && ok_ecc) {
        printf("[PASS] ExportEnc unified: RSA+ECC all passed\n");
        return 0;
    }

    fprintf(stderr, "[FAIL] ExportEnc unified summary rsa=%d ecc=%d\n", ok_rsa, ok_ecc);
    return 1;
}
