...
static int test_rsa(HANDLE sess)
{
    ...
    print_sha256_hex("EPK.RSA.ct", out, out_len);
...
static int test_ecc(HANDLE sess)
{
    ...
    print_sha256_hex("EPK.ECC.ct", out, out_len);
    (void)SDF_DestroyKey(sess, key);
    free(out);
...
static int test_pq(HANDLE sess)
{
    ...
    if (rc != SDR_OUTARGERR || ct_len == 0) {
        fprintf(stderr, "[FAIL] unified EPK PQ size-probe rc=0x%08X len=%u\n", (unsigned)rc, (unsigned)ct_len);
        goto pq_cleanup;
    }
    ...
    print_sha256_hex("EPK.ct", ct, ct_len);
    print_sha256_hex("EPK.ss", ss_dec, ss_dec_len);
